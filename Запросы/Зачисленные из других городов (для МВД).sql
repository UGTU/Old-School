...
where (pr.Ik_raion in (46,41))or(fac.Ik_raion in (46,41))or(vrem.Ik_raion in (46,41))or(pr.Ik_region = 69)or(fac.Ik_region = 69)or(vrem.Ik_region = 69)
order by FIO
-------------------------------------------------------------------------------------------------------------------------------------
--абитуриенты
select FIO,Cname_fac,Cname_spec, pr.Cstrana + ', ' + pr.Cregion + ', ' + pr.Craion +
	   ', ' + pr.Cgorod + ', ' + pr.CStreet+', '+pr.BuildingNumber+' - '+pr.FlatNumber,
		fac.Cstrana + ', ' + fac.Cregion + ', ' + fac.Craion + ', ' + fac.Cgorod + ', ' + fac.CStreet+', '+fac.BuildingNumber+' - '+fac.FlatNumber, 
		uht.Cstrana + ', ' + uht.Cregion + ', ' + uht.Craion + ', ' + uht.Cgorod + ', ' + uht.CStreet+', '+uht.BuildingNumber+' - '+uht.FlatNumber,
		Dd_birth, Cname_grup
from
(

Select distinct Fac.Cname_fac,Spec_stud.Cname_spec, Clastname + ' ' + Cfirstname + ' ' + Cotch FIO,
	   dbo.Person.nCode,NNyear,Cname_form_ed,CType_kat, Person.ctelefon,Person.cSotTel, cast(Person.Dd_birth AS DATE) Dd_birth
...
and Relation_spec_fac.Ik_form_ed = 1									--не заочники
and NNyear=2013
) Allstud
left join (select Grup.Cname_grup, Zach.nCode
		   from Grup, StudGrup, Zach
		   where Grup.Ik_grup = StudGrup.Ik_grup and StudGrup.Ik_zach = Zach.Ik_zach
		   and StudGrup.Ik_prikazOtch is null
		   ) gr on gr.nCode = Allstud.nCode
left join (select nCode,FlatNumber,StructNumber,BuildingNumber,CStreet,Cgorod, Craion, Cregion,Cstrana,Strana.Ik_strana,Raion.Ik_raion, Region.Ik_region
		   from dbo.PersonAddress,dbo.Address,dbo.Street,dbo.Gorod,dbo.Raion,dbo.Region,dbo.Strana
		   where ik_AddressType=2
		   and dbo.Address.ik_address = dbo.PersonAddress.ik_address
		   and dbo.Address.Ik_street = dbo.Street.Ik_street
...
left join (select nCode,FlatNumber,StructNumber,BuildingNumber,CStreet,Cgorod,Craion,Cregion,Cstrana,Strana.Ik_strana,Raion.Ik_raion, Region.Ik_region
		   from dbo.PersonAddress,dbo.Address,dbo.Street,dbo.Gorod,dbo.Raion,dbo.Region,dbo.Strana
		   where dbo.Gorod.Ik_raion = 45
		   and dbo.Address.ik_address = dbo.PersonAddress.ik_address
		   and dbo.Address.Ik_street = dbo.Street.Ik_street
		   and dbo.Street.ik_gorod = dbo.Gorod.Ik_gorod
		   and dbo.Gorod.Ik_raion = dbo.Raion.Ik_raion
		   and dbo.Raion.Ik_region = dbo.Region.Ik_region
		   and dbo.Region.Ik_strana = dbo.Strana.Ik_strana) uht
on Allstud.nCode = uht.nCode
/*left join (select nCode,FlatNumber,StructNumber,BuildingNumber,CStreet,Cgorod,Cregion,Cstrana,Strana.Ik_strana,Raion.Ik_raion, Region.Ik_region
		   from dbo.PersonAddress,dbo.Address,dbo.Street,dbo.Gorod,dbo.Raion,dbo.Region,dbo.Strana
		   where ik_AddressType=3
		   and dbo.Address.ik_address = dbo.PersonAddress.ik_address
		   ...
		   and dbo.Region.Ik_strana = dbo.Strana.Ik_strana) vrem
on Allstud.nCode = vrem.nCode*/
where (pr.Ik_raion <> 45)or(fac.Ik_raion <>45)
