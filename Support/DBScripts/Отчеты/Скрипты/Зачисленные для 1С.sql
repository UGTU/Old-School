...
left join 
(SELECT distinct Rodit_stud.nCode, MAX(Rodit_stud.Cfio_rods) Cfio_rods
from dbo.Rodit_stud
GROUP BY Rodit_stud.nCode

)Rodit_stud


 on Rodit_stud.nCode=stud.nCode
--left join dbo.memb_fam on Rodit_stud.Ik_memb_fam=memb_fam.cmemb_fam
--where (pr.Cstrana is null)and(vrem.Cstrana is null)
--where Dd_vidan is null
--where stud.nCode = 113819
--where Rodit_stud.Ik_memb_fam in (,,,,,,,,,,)
