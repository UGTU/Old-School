select Cshort_name_fac, Cname_spec, Sh_spec, ISNULL(Grup.Cname_grup,'') GrupName, CType_kat,Cname_form_ed,FIO,c_grazd,
	Nn_prikaz, CONVERT(VARCHAR(10),Dd_prikaz,104) prData from
(

/*фамилия, имя, отчество, гражданство, дата рождения, учебная группа (обязательно!), направление подготовки, 
шифр направления подготовки, номер приказа по зачислению, дата приказа по зачислению, форма обучения (очно, заочно), 
основа обучения (бюджет, контракт, квоты).*/

select distinct Fac.Cshort_name_fac,Spec_stud.Cname_spec, PersonFullName FIO,
	   dbo.Person.nCode,grazd.Ik_grazd,c_grazd,NNyear,Cname_form_ed,CType_kat, Spec_stud.Sh_spec,
	   Prikaz.Nn_prikaz, Prikaz.Dd_prikaz
from dbo.Person,ABIT_postup,ABIT_Diapazon_spec_fac,dbo.Student,Relation_spec_fac,Spec_stud,Fac,
	 grazd,Form_ed,Kat_zach,TypeKatZach, dbo.Prikaz
...
left join 
dbo.Zach ON stud.nCode=Zach.nCode
left join dbo.StudGrup ON Zach.Ik_zach=StudGrup.Ik_zach and StudGrup.Ik_prikazOtch is null
left join dbo.Grup ON StudGrup.ik_grup=Grup.Ik_grup
