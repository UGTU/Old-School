...
where (pr.Ik_raion in (45,99,35,41,21,15))--or(fac.Ik_raion in (45))or(vrem.Ik_raion in (45))
order by FIO
-------------------------------------------------------------------------------------------------------------------------------------
--абитуриенты
select FIO,Cname_fac,Cname_spec,Cname_form_ed,sum_b
...
Select distinct ABIT_postup.NN_abit,Fac.Cname_fac,Spec_stud.Cname_spec, Clastname + ' ' + Cfirstname + ' ' + Cotch FIO, sum(ABIT_Vstup_exam.cosenka) sum_b,
...
group by ABIT_postup.NN_abit,Fac.Cname_fac,Spec_stud.Cname_spec, Clastname + ' ' + Cfirstname + ' ' + Cotch,
	   dbo.Person.nCode,grazd.Ik_grazd,c_grazd,NNyear,cshort_sdach,Cname_form_ed,CType_kat, Person.ctelefon,Person.cSotTel
) Allstud
left join (select nCode,FlatNumber,StructNumber,BuildingNumber,CStreet,Cgorod,Cregion,Cstrana,Strana.Ik_strana,Raion.Ik_raion, Region.Ik_region,
			Gorod.Ik_gorod
...
left join (select nCode,FlatNumber,StructNumber,BuildingNumber,CStreet,Cgorod,Cregion,Cstrana,Strana.Ik_strana,Raion.Ik_raion, Region.Ik_region,
		   ...
		   and dbo.Region.Ik_strana = dbo.Strana.Ik_strana) vrem
on Allstud.nCode = vrem.nCode
where (pr.Ik_strana in (748))or(fac.Ik_strana in (748))or(vrem.Ik_strana in (748))
order by FIO
