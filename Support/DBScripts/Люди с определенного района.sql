select NNyear,FIO,CType_kat,cshort_sdach,Cname_form_ed,pr.Cstrana + ', ' + pr.Cregion +
...
left join (select nCode,FlatNumber,StructNumber,BuildingNumber,CStreet,Cgorod,Cregion,Cstrana,Strana.Ik_strana,Raion.Ik_raion
...
where (pr.Ik_raion = 14)or(fac.Ik_raion = 14)or(vrem.Ik_raion = 14)
order by NNyear,FIO
