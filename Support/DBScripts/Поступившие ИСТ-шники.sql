select * from ABIT_Diapazon_spec_fac 
where ik_spec_fac in (select ik_spec_fac from Relation_spec_fac 
					  where ik_spec = 354549 and Ik_form_ed=1)
and NNyear = 2012

---------------------------------------------------------------------------------------------------------
select FIO,c_grazd,Dd_birth, cEmail, Lrab, Cname_kat_zach,cSotTel,ctelefon,pr.Cstrana + ', ' + pr.Cregion +
...
select distinct Fac.Cshort_name_fac,Spec_stud.Cname_spec, Clastname + ' ' + Cfirstname + ' ' + Cotch FIO,
	   ...
	   cast(Dd_birth as DATE) Dd_birth, cEmail, Lrab
