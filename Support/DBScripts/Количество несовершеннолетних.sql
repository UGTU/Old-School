...
select distinct Person.nCode,Clastname + ' ' + Cfirstname + iif(Cotch is not null, ' ' + Cotch,''), Grup.Cname_grup, Cshort_name_fac, cast(Dd_birth as date), Cname_spec,  
	   Gorod.Cgorod + ', ' + Street.CStreet + ' ' + [BuildingNumber]  + iif(([StructNumber] is not null)and([StructNumber]!=''), ', корп.' + [StructNumber] ,'') + iif([FlatNumber] is not null, ', кв. ' + [FlatNumber], '')
from person
left join ABIT_postup on person.nCode = ABIT_postup.nCode
left join Zach on Zach.nCode = person.nCode
left join StudGrup on StudGrup.Ik_zach = Zach.Ik_zach
...
left join Raion on Gorod.[Ik_raion] =  Raion.Ik_raion
...
--and Fac.ik_fac not in (15,17)
order by Clastname + ' ' + Cfirstname + iif(Cotch is not null, ' ' + Cotch,'')