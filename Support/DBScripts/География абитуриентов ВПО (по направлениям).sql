DECLARE @temp_abit	TABLE
	(
		FIO				varchar(200),
		Cshort_name_fac	varchar(10),
		ik_type_kat		int,
		CType_kat		varchar(20),
		Ik_form_ed		int,
		Cname_form_ed	varchar(20),
		ik_spec			int,
		Cname_spec		varchar(200),
		p_region		int,
		f_region		int,
		v_region		int,
		name_region		varchar(200),
		name_strana		varchar(200)
	)
insert into @temp_abit
--абитуриенты
select FIO,Cshort_name_fac,ik_type_kat,CType_kat,Ik_form_ed,Cname_form_ed,ik_spec,Cname_spec,pr.Ik_region, fac.Ik_region, vrem.Ik_region,'',''
from
(

Select distinct Fac.Cshort_name_fac,Spec_stud.Cname_spec, Clastname + ' ' + Cfirstname + ' ' + Cotch FIO, Spec_stud.ik_spec,
	   dbo.Person.nCode,NNyear,cshort_sdach,CType_kat,TypeKatZach.ik_type_kat,Form_ed.Ik_form_ed,Form_ed.Cname_form_ed
from dbo.Person,ABIT_postup,ABIT_Diapazon_spec_fac,dbo.Student,Relation_spec_fac,Spec_stud,Fac,ABIT_Vstup_exam,
	 ABIT_VidSdachi,Kat_zach,TypeKatZach, Form_ed
...
select Cshort_name_fac,ik_spec,Cname_spec,Ik_form_ed,Cname_form_ed,ik_type_kat,CType_kat,p_region,name_region,name_strana,count(FIO)
from @temp_abit
group by ik_type_kat,CType_kat,Ik_form_ed,Cname_form_ed,ik_spec,Cname_spec,Cshort_name_fac,p_region, name_region,name_strana
