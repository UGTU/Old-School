--студе
select FIO,Cname_fac,Cname_spec,Course,Cname_grup,Cname_form_ed, pr.Cgorod, fac.Cgorod, vrem.Cgorod,
...
(Select distinct Person.nCode,Fac.Cname_fac,Spec_stud.Cname_spec,Form_ed.Cname_form_ed, cast((year(GETDATE())-Grup.nYear_post+1) as varchar(2)) + ' ' + 'курс' as Course,
...
where (pr.Ik_raion in (46))or(fac.Ik_raion in (46))or(vrem.Ik_raion in (46))
order by FIO
-------------------------------------------------------------------------------------------------------------------------------------
--абитуриенты
select pr.Cgorod, fac.Cgorod, vrem.Cgorod, FIO,Cname_fac,Cname_spec,Cname_form_ed
	/*'1 курс',pr.Cstrana + ', ' + pr.Cregion +
...
left join (select nCode,FlatNumber,StructNumber,BuildingNumber,CStreet,Cgorod,Cregion,Cstrana,Strana.Ik_strana,Raion.Ik_raion, Region.Ik_region,
		   ...
		   and dbo.Region.Ik_strana = dbo.Strana.Ik_strana) vrem
on Allstud.nCode = vrem.nCode
where (pr.Ik_strana in (45,99,35,41,21,15))or(fac.Ik_strana in (45,99,35,41,21,15))or(vrem.Ik_strana in (45,99,35,41,21,15))
order by FIO
