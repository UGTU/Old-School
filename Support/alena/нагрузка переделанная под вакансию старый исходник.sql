...
/****** Object:  View [TeachersPlan].[НагрузкаT]    Script Date: 20.04.2016 11:09:34 ******/
SET ANSI_NULLS ON
GO
...
CREATE view [TeachersPlan].[НагрузкаT]
(ПланФакт,
idFactStaffHistory,
...
IIF ([TeachersPlan].[DisciplineLoad].[Quantity] is not null,[TeachersPlan].[DisciplineLoad].[Quantity],[TeachersPlan].[DisciplineLoad].[factQuantity]) as[Часов],
	...
	FROM 
	TeachersPlan.IndividualPlans
	inner join Import.KafTeachers
	on Import.KafTeachers.idFactStaffHistory =TeachersPlan.IndividualPlans.IdTeacher  
    inner join 
	  (select distinct [КодПреподавателя], [dekanatMMIS].[Import].[Educators].[DepartmentGUID], [dekanatMMIS].[Import].[Educators].[EmployeeGUID]  
	...
	inner join 
	  (select max(TeachersPlan.ImportCurricularLoad.IDImportCurricularLoad) as  IDImportCurricularLoad, [dekanatMMIS].[Import].[Educators].idFactStaffHistory
	   from [dekanatMMIS].[Import].[Educators] inner join
	   TeachersPlan.IndividualPlans on [dekanatMMIS].[Import].[Educators].idFactStaffHistory = TeachersPlan.IndividualPlans.IdTeacher
			   ...
			   group by  [dekanatMMIS].[Import].[Educators].idFactStaffHistory
