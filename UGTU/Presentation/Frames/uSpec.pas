...
uses
  ...
  ADODB, Grids, DBGridEh, uUchPlan, ImgList, ActnList, DateUtils,
  UchPlanController,
...
begin
  ...
  TabSheet2.TabVisible := (VidGos > FGOS2);
  // скрыть вкладку ФГОС для старых специальностей
  dsSpclz.DataSet := TADODataSet.Create(nil);
  TUchPlanController.Instance.getAllSpecializations(@dsSpclz.DataSet,
    SpecIK, false);
...
begin
  if (dsSpclz.DataSet <> nil) then
    ...
    begin
      ...
      frmMain.StatusBar1.Panels[1].Text := spclz + dsSpclz.DataSet.FieldByName
        ('cName_spclz_short').AsString;
    end;
end;
  ...
  try
    ...
    begin
      ...
      frmMain.DBDekTreeView_TEST1.RefreshNodeExecute
        (frmMain.DBDekTreeView_TEST1.Selected);
    end;
  finally
    frmGroupEdt.Free;
  end;
...
begin
  inherited;
  if MessageDlg('Вы уверены что хотите удалить выбранные группы?',
    mtConfirmation, [mbYes, mbNo], 0) = mrYes then
  begin
    for i := 0 to DBGridEh1.SelectedRows.Count - 1 do
    begin
      DBGridEh1.DataSource.DataSet.GotoBookmark
        (Pointer(DBGridEh1.SelectedRows[i]));
...
begin
  if (DataSource1.DataSet <> nil) then
    if (DataSource1.DataSet.Active) then
      frmMain.StatusBar1.Panels[1].Text := 'Группа: ' +
        DataSource1.DataSet.FieldByName('Cname_grup').AsString;
end;
...
begin
  inherited;
  frmSpecAddSpclz := TfrmSpecAddSpclz.CreateDialog(Application,
    Connection, nil);
  ...
  begin
    ...
    frmSpecAddSpclz.dbeShSpclz.Text := dsSpclz.DataSet.FieldByName
      ('Csh_spclz').AsString;
    frmSpecAddSpclz.dbeName.Text := dsSpclz.DataSet.FieldByName
      ('cName_spclz').AsString;
    frmSpecAddSpclz.dbeNameShort.Text := dsSpclz.DataSet.FieldByName
      ('cName_spclz_short').AsString;
  end;
...
begin
  ...
  begin
    ...
    tempStoredProc.Parameters.CreateParameter('@ik_spclz', ftInteger, pdInput,
      0, dbgSpclz.DataSource.DataSet.FieldByName('iK_spclz').AsInteger);
...
begin
  case PageControl1.ActivePageIndex of
    ...
    1:
      begin
        TApplicationController.GetInstance.AddLogEntry
          ('Переход на список специализаций');
        if (dsSpclz.DataSet <> nil) then
          if (dsSpclz.DataSet.Active) then
            frmMain.StatusBar1.Panels[1].Text := 'Специализация: ' +
              dsSpclz.DataSet.FieldByName('cName_spclz_short').AsString;
      end;
    2:
      if (VidGos > FGOS2) then
      begin
        TApplicationController.GetInstance.AddLogEntry
          ('Переход на список учебных планов');
        if ((fmUchPlan1.dbcbSpclz.KeyValue <> Null) and
          (fmUchPlan1.dbcbFormEd.KeyValue <> Null) and
          (fmUchPlan1.dbcbYear.KeyValue <> Null)) then
          frmMain.StatusBar1.Panels[1].Text := 'Учебный план: ' +
            fmUchPlan1.dbcbSpclz.ListSource.DataSet.FieldByName
            ('cName_spclz_short').AsString + ', ' +
            AnsiLowerCase(fmUchPlan1.dbcbFormEd.Text) + ', ' +
            fmUchPlan1.dbcbYear.Text
        else
    ...
    3:
      begin
        ...
        dmStudentData.adodsAcadem.CommandText :=
          'select * from AcademStud where [Ik_spec]=' + inttostr(SpFacik);
        dmStudentData.adodsAcademStringField11.Visible := false;
        dmStudentData.adodsAcadem.Active := true;
        frmMain.Comment(TDBNodeSpecObject(FrameObject).Name,
          'Академических отпусков: ' +
          inttostr(dmStudentData.adodsAcadem.RecordCount));
        fmAcadem.FrameLevel := 3;
        fmAcadem.FrameObject := FrameObject;
      end;
    4:
      begin
        frmMain.Comment('Средние баллы студентов', 'Направление подготовки ' +
          (FrameObject as TDBNodeSpecObject).Name);
        ...
        fmAverageBallsSpec.ShowStudents := (FrameObject as TDBNodeSpecObject)
          .ShowStudents;
        fmAverageBallsSpec.StudyYears := (FrameObject as TDBNodeSpecObject)
          .StudyYears;
        fmAverageBallsSpec.ShowActualGroup := (FrameObject as TDBNodeSpecObject)
          .showActual;
