inherited frmSheduleSetRoomConstraints: TfrmSheduleSetRoomConstraints
  Caption = #1054#1075#1088#1072#1085#1080#1095#1077#1085#1080#1077' '#1087#1086' '#1080#1089#1087#1086#1083#1100#1079#1086#1074#1072#1085#1080#1102' '#1072#1091#1076#1080#1090#1086#1088#1085#1086#1075#1086' '#1092#1086#1085#1076#1072
  ClientHeight = 350
  ClientWidth = 525
  ExplicitWidth = 531
  ExplicitHeight = 382
  PixelsPerInch = 96
  TextHeight = 13
  inherited Bevel1: TBevel
    Top = 308
    Width = 525
    Height = 7
    Align = alBottom
    Shape = bsBottomLine
    Style = bsLowered
    ExplicitTop = 139
    ExplicitWidth = 478
    ExplicitHeight = 7
  end
  object Label4: TLabel [1]
    Left = 8
    Top = 8
    Width = 165
    Height = 13
    Caption = #1047#1072#1087#1080#1089#1100' '#1087#1083#1072#1085#1072' '#1091#1095#1077#1073#1085#1086#1081' '#1085#1072#1075#1088#1091#1079#1082#1080':'
  end
  object Label5: TLabel [2]
    Left = 179
    Top = 8
    Width = 3
    Height = 13
    Anchors = [akLeft, akTop, akRight]
    Caption = '-'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clMaroon
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
    ParentShowHint = False
    ShowHint = True
  end
  inherited Panel1: TPanel
    Top = 315
    Width = 525
    Height = 35
    BevelOuter = bvNone
    ExplicitTop = 315
    ExplicitWidth = 525
    ExplicitHeight = 35
    object Image1: TImage [0]
      Left = 87
      Top = 9
      Width = 21
      Height = 16
      Cursor = crHandPoint
      Anchors = [akLeft, akBottom]
      Picture.Data = {
        
        00030000}
      OnClick = actSpravExecute
      OnMouseEnter = Label7MouseEnter
      OnMouseLeave = Label7MouseLeave
    end
    object Image9: TImage [1]
      Left = 8
      Top = 9
      Width = 21
      Height = 16
      Cursor = crHandPoint
      Anchors = [akLeft, akBottom]
      Picture.Data = {
        
        F00F1CFF}
      OnClick = Label17Click
      OnMouseEnter = Label17MouseEnter
      OnMouseLeave = Label17MouseLeave
    end
    object Label17: TLabel [2]
      Left = 31
      Top = 11
      Width = 43
      Height = 13
      Cursor = crHandPoint
      Anchors = [akLeft, akBottom]
      Caption = #1057#1087#1088#1072#1074#1082#1072
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clNavy
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      OnClick = Label17Click
      OnMouseEnter = Label17MouseEnter
      OnMouseLeave = Label17MouseLeave
    end
    object Label7: TLabel [3]
      Left = 110
      Top = 11
      Width = 67
      Height = 13
      Cursor = crHandPoint
      Anchors = [akLeft, akBottom]
      Caption = #1057#1087#1088#1072#1074#1086#1095#1085#1080#1082#1080
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clNavy
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      OnClick = actSpravExecute
      OnMouseEnter = Label7MouseEnter
      OnMouseLeave = Label7MouseLeave
    end
    inherited bbOK: TBitBtn
      Left = 272
      Top = 5
      ExplicitLeft = 272
      ExplicitTop = 5
    end
    inherited bbApply: TBitBtn
      Left = 436
      Top = 5
      ExplicitLeft = 436
      ExplicitTop = 5
    end
    inherited bbCancel: TBitBtn
      Left = 354
      Top = 5
      ExplicitLeft = 354
      ExplicitTop = 5
    end
    inherited bbSprav: TBitBtn
      Left = 27
      Top = 29
      Width = 113
      Visible = False
      ExplicitLeft = 27
      ExplicitTop = 29
      ExplicitWidth = 113
    end
  end
  object RadioButton1: TRadioButton [4]
    Left = 8
    Top = 52
    Width = 132
    Height = 17
    Caption = #1040#1091#1076#1080#1090#1086#1088#1080#1080' '#1082#1072#1092#1077#1076#1088#1099':'
    TabOrder = 1
    OnClick = RadioButton1Click
  end
  object StaticText1: TStaticText [5]
    Left = 8
    Top = 30
    Width = 497
    Height = 17
    AutoSize = False
    Caption = 
      #1059#1082#1072#1078#1080#1090#1077' '#1074' '#1082#1072#1082#1080#1093' '#1072#1091#1076#1080#1090#1086#1088#1080#1103#1093' '#1085#1077#1086#1073#1093#1086#1076#1080#1084#1086' '#1087#1088#1086#1074#1086#1076#1080#1090#1100' '#1079#1072#1085#1103#1090#1080#1103', '#1089#1086#1086#1090#1074#1077#1090 +
      #1089#1090#1074#1091#1102#1097#1080#1077' '#1076#1072#1085#1085#1086#1081' '#1079#1072#1087#1080#1089#1080':'
    TabOrder = 2
  end
  object RadioButton2: TRadioButton [6]
    Left = 8
    Top = 74
    Width = 217
    Height = 17
    Caption = #1040#1091#1076#1080#1090#1086#1088#1080#1080' '#1080#1079' '#1089#1083#1077#1076#1091#1102#1097#1077#1075#1086' '#1089#1087#1080#1089#1082#1072':'
    Checked = True
    TabOrder = 3
    TabStop = True
    OnClick = RadioButton1Click
  end
  object Panel2: TPanel [7]
    Left = 17
    Top = 96
    Width = 505
    Height = 211
    Anchors = [akLeft, akTop, akRight, akBottom]
    BevelOuter = bvNone
    TabOrder = 4
    object Splitter1: TSplitter
      Left = 303
      Top = 0
      Height = 211
      ExplicitLeft = 112
      ExplicitTop = 56
      ExplicitHeight = 100
    end
    object Panel3: TPanel
      Left = 0
      Top = 0
      Width = 303
      Height = 211
      Align = alLeft
      BevelOuter = bvNone
      TabOrder = 0
      DesignSize = (
        303
        211)
      object Label1: TLabel
        Left = 8
        Top = 22
        Width = 39
        Height = 13
        Caption = #1050#1086#1088#1087#1091#1089':'
      end
      object Label2: TLabel
        Left = 8
        Top = 0
        Width = 115
        Height = 13
        Caption = #1044#1086#1089#1090#1091#1087#1085#1099#1077' '#1072#1091#1076#1080#1090#1086#1088#1080#1080':'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object dbcbCampus: TDBLookupComboboxEh
        Left = 53
        Top = 20
        Width = 70
        Height = 19
        AlwaysShowBorder = True
        EditButtons = <>
        Flat = True
        TabOrder = 0
        Visible = True
        OnKeyValueChanged = dbcbCampusKeyValueChanged
      end
      object DBGridEh1: TDBGridEh
        Left = 8
        Top = 44
        Width = 294
        Height = 166
        Anchors = [akLeft, akTop, akRight, akBottom]
        AutoFitColWidths = True
        DataSource = dsCampusRooms
        Flat = False
        FooterColor = clWindow
        FooterFont.Charset = DEFAULT_CHARSET
        FooterFont.Color = clWindowText
        FooterFont.Height = -11
        FooterFont.Name = 'MS Sans Serif'
        FooterFont.Style = []
        Options = [dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit, dgMultiSelect]
        TabOrder = 1
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        Columns = <
          item
            EditButtons = <>
            FieldName = 'full_name'
            Footers = <>
            Title.Caption = #1053#1072#1079#1074#1072#1085#1080#1077
            Width = 59
          end
          item
            EditButtons = <>
            FieldName = 'cName_room_type'
            Footers = <>
            Title.Caption = #1058#1080#1087' '#1072#1091#1076#1080#1090#1086#1088#1080#1080
            Width = 135
          end
          item
            EditButtons = <>
            FieldName = 'iSeatCount'
            Footers = <>
            Title.Caption = #1063#1080#1089#1083#1086' '#1084#1077#1089#1090
            Width = 53
          end>
      end
    end
    object Panel4: TPanel
      Left = 306
      Top = 0
      Width = 199
      Height = 211
      Align = alClient
      BevelOuter = bvNone
      TabOrder = 1
      DesignSize = (
        199
        211)
      object Label3: TLabel
        Left = 35
        Top = 0
        Width = 95
        Height = 13
        Caption = #1057#1087#1080#1089#1086#1082' '#1072#1091#1076#1080#1090#1086#1088#1080#1081':'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object sbAddMany: TSpeedButton
        Left = 4
        Top = 78
        Width = 25
        Height = 25
        Cursor = crHandPoint
        Hint = #1044#1086#1073#1072#1074#1080#1090#1100' '#1072#1091#1076#1080#1090#1086#1088#1080#1080
        Enabled = False
        Flat = True
        Glyph.Data = {
          
          FFFF00FFFF00FF000000}
        ParentShowHint = False
        ShowHint = True
        OnClick = sbAddManyClick
      end
      object sbAddOne: TSpeedButton
        Left = 4
        Top = 103
        Width = 25
        Height = 25
        Cursor = crHandPoint
        Hint = #1044#1086#1073#1072#1074#1080#1090#1100' '#1072#1091#1076#1080#1090#1086#1088#1080#1102
        Action = actAddOne
        Flat = True
        Glyph.Data = {
          
          FFFF00FFFF00FF000000}
        ParentShowHint = False
        ShowHint = True
      end
      object sbDelMany: TSpeedButton
        Left = 4
        Top = 153
        Width = 25
        Height = 25
        Cursor = crHandPoint
        Hint = #1059#1076#1072#1083#1080#1090#1100' '#1072#1091#1076#1080#1090#1086#1088#1080#1080
        Enabled = False
        Flat = True
        Glyph.Data = {
          
          FFFF00FFFF00FF000000}
        ParentShowHint = False
        ShowHint = True
        OnClick = sbDelManyClick
      end
      object sbDelOne: TSpeedButton
        Left = 4
        Top = 128
        Width = 25
        Height = 25
        Cursor = crHandPoint
        Hint = #1059#1076#1072#1083#1080#1090#1100' '#1072#1091#1076#1080#1090#1086#1088#1080#1102
        Enabled = False
        Flat = True
        Glyph.Data = {
          
          FFFF00FFFF00FF000000}
        ParentShowHint = False
        ShowHint = True
        OnClick = sbDelOneClick
      end
      object ListBox2: TListBox
        Left = 35
        Top = 20
        Width = 161
        Height = 190
        Anchors = [akLeft, akTop, akRight, akBottom]
        ItemHeight = 13
        MultiSelect = True
        TabOrder = 0
      end
    end
  end
  object dbcbKaf: TDBLookupComboboxEh [8]
    Left = 139
    Top = 50
    Width = 126
    Height = 19
    AlwaysShowBorder = True
    EditButtons = <>
    Flat = True
    TabOrder = 5
    Visible = True
  end
  inherited actBaseDialog: TActionList
    Left = 345
    Top = 40
    inherited actApply: TAction
      OnUpdate = actApplyUpdate
    end
    inherited actOK: TAction
      OnUpdate = actOKUpdate
    end
    object actAddOne: TAction
      Enabled = False
      OnExecute = actAddOneExecute
      OnUpdate = actAddOneUpdate
    end
  end
  object dsCampusRooms: TDataSource
    Left = 232
    Top = 176
  end
end
