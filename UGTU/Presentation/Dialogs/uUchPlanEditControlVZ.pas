...
uses
  ...
  Grids, Menus, DBGridEh, ImgList, UchPlanController, GridsEh, ComCtrls,
...
type
  ...
  public
    procedure Read(defaultKafIK, DiscInUchPlanIK, vidZanyatIK: integer;
      vidZanyatName: string);
  end;
...
procedure TfrmEditControlVZ.Read(defaultKafIK, DiscInUchPlanIK,
  vidZanyatIK: integer; vidZanyatName: string);
begin
...
begin
  if (dbgContent.DataSource.DataSet.State = dsInsert) or
    (dbgContent.DataSource.DataSet.State = dsEdit) then
...
begin
  case (Application.MessageBox('Сохранить внесенные изменения?',
    'Изменение параметров дисциплины учебного плана', MB_YESNOCANCEL)) of
    mrYes:
      begin
        OK;
        Result := false;
...
begin
  try
    ...
    begin
      if (dbgContent.DataSource.DataSet.FieldByName('n_sem').AsInteger > max)
      then
