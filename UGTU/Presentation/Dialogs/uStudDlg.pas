...
begin
  ...
  begin
    if eFam = nil then
      exit;
    if (eFam.Text = '') or (eName.Text = '') or (eNum.Text = '') or
      (eNum.Text = '      ') or (dbdteBirthDate.Text = '  .  .    ') or
      (dbcbeSex.Text = '') or (dbcbeCitizenship.Text = '') or
      (dbcbeSchool.Text = '') or (dbcbeMedal.Text = '') or (dbcbeCause.Text = '')
    then
      result := false;
  end;
end;
