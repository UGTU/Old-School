inherited frmAcademReturn: TfrmAcademReturn
  Left = 158
  Top = 307
  Caption = #1042#1086#1079#1074#1088#1072#1097#1077#1085#1080#1077' '#1080#1079' '#1072#1082#1072#1076#1077#1084#1080#1095#1077#1089#1082#1086#1075#1086' '#1086#1090#1087#1091#1089#1082#1072
  ClientHeight = 243
  ClientWidth = 307
  OldCreateOrder = True
  OnShow = FormShow
  ExplicitWidth = 313
  ExplicitHeight = 275
  PixelsPerInch = 96
  TextHeight = 13
  inherited Bevel1: TBevel
    Width = 307
    Height = 202
    ExplicitWidth = 307
    ExplicitHeight = 202
  end
  object Label2: TLabel [1]
    Left = 11
    Top = 10
    Width = 56
    Height = 13
    Caption = #8470' '#1087#1088#1080#1082#1072#1079#1072
  end
  object Label57: TLabel [2]
    Left = 73
    Top = 10
    Width = 4
    Height = 13
    Caption = '*'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clRed
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
  end
  object Label4: TLabel [3]
    Left = 11
    Top = 52
    Width = 68
    Height = 13
    Caption = #1054#1073#1086#1089#1085#1086#1074#1072#1085#1080#1077
  end
  object Label1: TLabel [4]
    Left = 12
    Top = 170
    Width = 68
    Height = 13
    Caption = #1054#1073#1086#1089#1085#1086#1074#1072#1085#1080#1077
  end
  object sbAddPrikaz: TSpeedButton [5]
    Left = 273
    Top = 26
    Width = 24
    Height = 23
    Hint = #1044#1086#1073#1072#1074#1080#1090#1100' '#1087#1088#1080#1082#1072#1079'...'
    Flat = True
    Glyph.Data = {
      
      EF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
    OnClick = sbAddPrikazClick
  end
  inherited Panel1: TPanel
    Top = 202
    Width = 307
    ExplicitTop = 202
    ExplicitWidth = 307
    inherited bbOK: TBitBtn
      Left = 131
      Top = 9
      ExplicitLeft = 131
      ExplicitTop = 9
    end
    inherited bbApply: TBitBtn
      Left = 218
      Visible = False
      ExplicitLeft = 218
    end
    inherited bbCancel: TBitBtn
      Left = 216
      Top = 9
      ExplicitLeft = 216
      ExplicitTop = 9
    end
    inherited bbSprav: TBitBtn
      Visible = False
    end
  end
  object dbcbeNumPrik: TDBLookupComboboxEh [7]
    Left = 11
    Top = 28
    Width = 252
    Height = 19
    EditButtons = <>
    Flat = True
    KeyField = 'Ik_prikaz'
    ListField = 'NN_Date'
    ListSource = dmPrikaz.dsPrikaz
    TabOrder = 1
    Visible = True
    OnChange = dbcbeNumPrikChange
  end
  object ezachObosn: TMemo [8]
    Left = 11
    Top = 69
    Width = 286
    Height = 121
    BevelInner = bvNone
    BorderStyle = bsNone
    MaxLength = 1000
    TabOrder = 2
  end
end
