...
begin
  if DlgOwner then
  begin
    adr := TaddressRecord.Create(dbcbeAddressType.KeyValue,
      dbcbeStreet.KeyValue, eHouse.Text, eBuilding.Text, eFlat.Text,
      eIndex.Text, dbdteBegin.Text, dbdteEnd.Text, dbcbeAddressType.Text,
      dbcbeCountry.Text, dbcbeReg.Text, dbcbeDist.Text, dbcbePoint.Text,
      dbcbeStreet.Text);
    ...
    begin
      ...
      (Owner as TftmStudent).SyncAddress;
       if dbcbeCountry.KeyValue <> Russia then
...
begin
  actApply.Enabled := (dbcbeAddressType.Text <> '') and
    (dbcbeStreet.Text <> '');
end;
...
begin
  ...
  begin
    ...
    adodsGorod.Active := false;
    adodsStreet.Active := false;
    if dbcbeCountry.KeyValue <> NULL then
    begin
      adodsRegion.CommandText :=
        'select Region.Ik_strana,IK_Region,CRegion from Region  where Ik_strana='''
        + string(dbcbeCountry.KeyValue) + '''';
      adodsRegion.Active := true;
...
begin
  ...
  begin
    adodsGorod.Active := false;
    adodsStreet.Active := false;
    if dbcbeDist.KeyValue <> NULL then
    begin
      adodsGorod.CommandText :=
        'select Ik_raion,IK_gorod,CGorod,CIndex from Gorod  where Ik_raion=''' +
        string(dbcbeDist.KeyValue) + '''';
      adodsGorod.Active := true;
...
begin
  ...
  begin
    adodsStreet.Active := false;
    if dbcbePoint.KeyValue <> NULL then
    begin
      adodsStreet.CommandText :=
        'select Ik_street,IK_gorod,CStreet from Street where Ik_gorod=''' +
        string(dbcbePoint.KeyValue) + '''';
      adodsStreet.Active := true;
...
begin
  ...
  begin
    ...
    adodsGorod.Active := false;
    adodsStreet.Active := false;
    if dbcbeReg.KeyValue <> NULL then
    begin
      adodsRaion.CommandText :=
        'select Ik_raion,IK_Region,CRaion from Raion  where Ik_region=''' +
        string(dbcbeReg.KeyValue) + '''';
      adodsRaion.Active := true;
...
begin
  ...
  begin
    ...
    if dbcbeCountry.KeyValue < 0 then
      dbcbeCountry.KeyValue := 2;
    if dbcbeReg.KeyValue < 0 then
      dbcbeReg.KeyValue := 1;
    if dbcbeDist.KeyValue < 0 then
      dbcbeDist.KeyValue := 45;
    if dbcbePoint.KeyValue < 0 then
      dbcbePoint.KeyValue := 504;
