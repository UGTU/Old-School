...
uses
  ...
  ComCtrls,
  Forms, Dialogs, StdCtrls, GeneralController, DBLookupEh, Variants, ExcelXP,
  ComObj,
...
type
  ...
  public
    ...
    function getCompetenceForDiscUchPlan(IKDiscUchPlan, IKUchPlan,
      IDGos: Integer): TDataSet;
    ...
    function getCompetenceSpec(SpecIK, TypeCompetenceIK, FgosIK: Integer)
      : TDataSet;
    ...
    function getTypeCompetence(SourceDataSet: PDataSet;
      isShowFirst: boolean): Variant;

    // getLevel_competence - загружает все составляющие компетенций (знать, уметь, владеть)
    function getLevel_competence(SourceDataSet: PDataSet;
      isShowFirst: boolean): Variant;

    // getType_level_sformyr - загружает все типы уровней сформированности комп (пороговый, повышенный)
    function getType_level_sformyr(SourceDataSet: PDataSet;
      isShowFirst: boolean): Variant;

    // getDisciplines - загружает все дисциплины
    function getDisciplines(SourceDataSet: PDataSet;
      isShowFirst: boolean): Variant;

    { SaveFgos сохраняет ФГОС
      OperationType - тип операции: 1-вставка, 2 - обновление }
    function SaveFgos(OperationType: byte; var FgosIK, SpecIK, YearIK, NumFgos,
      Znach_ZE_IK, hWeekAllLes, hWeekAuditorLess, MinHolidayWeek,
      MaxHolidayWeek, WinterHolidayWeek: Integer; DateFgos: TDateTime): boolean;

    // SaveFgosException - сохраняет исключение по ЗЕ для текущего ФГОС
    function SaveFgosException(OperationType: byte; var FgosIK: Integer;
    ...
    function SaveCompetence(OperationType: byte;
      ZnachCompetence, ShName: String; IKTypeCompetence, IDGos,
      IKCompetence: Integer): boolean;
    ...
    function SaveStructureCompetence(OperationType: byte;
      DescriptionContent: string; IKStructureCompetence, IKCompetence,
      IKLevelCompetence: Integer): boolean;
    ...
    function SaveLevelSformyr(OperationType: byte; Value_priznak: string;
      IKLevelSformyr, IKCompetence, IKTypeLevelSformyr, IKLevelCompetence
      : Integer): boolean;
...
begin
  if (dmFgos.aspGetFgosBySpec.Active) then
    dmFgos.aspGetFgosBySpec.Close;
...
begin
  if (dmFgos.aspGetDiscBasedOn.Active) then
    dmFgos.aspGetDiscBasedOn.Close;
  dmFgos.aspGetDiscBasedOn.Parameters.ParamByName('@ik_disc_uch_plan').Value :=
    IKDiscUchPlan;
...
begin
  if (dmFgos.aspGetDiscMainStayFor.Active) then
    dmFgos.aspGetDiscMainStayFor.Close;
  dmFgos.aspGetDiscMainStayFor.Parameters.ParamByName('@ik_disc_uch_plan').Value
    := IKDiscUchPlan;
  dmFgos.aspGetDiscMainStayFor.Open;
  Result := dmFgos.aspGetDiscMainStayFor;
end;

function TFgosController.getCompetenceForDiscUchPlan(IKDiscUchPlan, IKUchPlan,
  IDGos: Integer): TDataSet;
begin
  if (dmFgos.aspGetCompForDicsUchPlan.Active) then
    dmFgos.aspGetCompForDicsUchPlan.Close;
  dmFgos.aspGetCompForDicsUchPlan.Parameters.ParamByName('@ik_disc_uch_plan')
    .Value := IKDiscUchPlan;
  dmFgos.aspGetCompForDicsUchPlan.Parameters.ParamByName('@ik_uch_plan').Value
    := IKUchPlan;
  dmFgos.aspGetCompForDicsUchPlan.Parameters.ParamByName('@IDGos')
    .Value := IDGos;
  dmFgos.aspGetCompForDicsUchPlan.Open;
  Result := dmFgos.aspGetCompForDicsUchPlan;
end;

function TFgosController.getFgosBySpecYearNum(SpecIK, YearIK, Num: Integer)
  : TDataSet;
begin
  if (dmFgos.aspGetFgosSpecYearNum.Active) then
    dmFgos.aspGetFgosSpecYearNum.Close;
  dmFgos.aspGetFgosSpecYearNum.Parameters.ParamByName('@ik_spec').Value
    := SpecIK;
...
function TFgosController.getCompetenceSpec(SpecIK, TypeCompetenceIK,
  FgosIK: Integer): TDataSet;
begin
...
begin
  if (dmFgos.aspGetFgosException.Active) then
    dmFgos.aspGetFgosException.Close;
...
begin
  if (dmFgos.aspGetAllAboutFgos.Active) then
    dmFgos.aspGetAllAboutFgos.Close;
...
begin
  if (dmFgos.aspGetDataTitle.Active) then
    dmFgos.aspGetDataTitle.Close;
...
function TFgosController.getTypeCompetence(SourceDataSet: PDataSet;
  isShowFirst: boolean): Variant;
begin
  Result := TGeneralController.Instance.getDataSetValues(SourceDataSet,
    'Select * From Type_competence', 'ik_type_competence', isShowFirst, NULL);
end;

function TFgosController.getLevel_competence(SourceDataSet: PDataSet;
  isShowFirst: boolean): Variant;
begin
  Result := TGeneralController.Instance.getDataSetValues(SourceDataSet,
    'Select * From Level_competence', 'ik_level_competence', isShowFirst, NULL);
end;

function TFgosController.getType_level_sformyr(SourceDataSet: PDataSet;
  isShowFirst: boolean): Variant;
begin
  Result := TGeneralController.Instance.getDataSetValues(SourceDataSet,
    'Select * From Type_level_sformyr', 'ik_type_level_sformyr',
    isShowFirst, NULL);
end;

function TFgosController.getDisciplines(SourceDataSet: PDataSet;
  isShowFirst: boolean): Variant;
begin
  Result := TGeneralController.Instance.getDataSetValues(SourceDataSet,
    'Select * From discpln Order By cName_disc', 'iK_disc', isShowFirst, NULL);
end;

function TFgosController.SaveFgosException(OperationType: byte;
  var FgosIK: Integer; Znach_ZE_IK, DiscplIK, ExceptionIK: Integer): boolean;
var
...
begin
  ...
  (DataSet as TADOStoredProc).Parameters.CreateParameter('@ik_exception_fgos',
    ftInteger, pdInput, 0, ExceptionIK);
  (DataSet as TADOStoredProc).Parameters.CreateParameter('@ik_znach_ZE',
    ftInteger, pdInput, 0, Znach_ZE_IK);
  (DataSet as TADOStoredProc).Parameters.CreateParameter('@ik_disc', ftInteger,
    pdInput, 0, DiscplIK);
  (DataSet as TADOStoredProc).Parameters.CreateParameter('@IDGos', ftInteger,
    pdInput, 0, FgosIK);
  try
    DataSet.Open;
...
function TFgosController.SaveFgos(OperationType: byte;
  var FgosIK, SpecIK, YearIK, NumFgos, Znach_ZE_IK, hWeekAllLes,
  hWeekAuditorLess, MinHolidayWeek, MaxHolidayWeek, WinterHolidayWeek: Integer;
...
begin
  ...
  (DataSet as TADOStoredProc).Parameters.CreateParameter('@ik_spec', ftInteger,
    pdInput, 0, SpecIK);
  (DataSet as TADOStoredProc).Parameters.CreateParameter('@ik_year', ftInteger,
    pdInput, 0, YearIK);
  (DataSet as TADOStoredProc).Parameters.CreateParameter('@NumFgos', ftInteger,
    pdInput, 0, NumFgos);
  (DataSet as TADOStoredProc).Parameters.CreateParameter('@DateFgos',
    ftDateTime, pdInput, 0, DateFgos);
  (DataSet as TADOStoredProc).Parameters.CreateParameter('@ik_znach_ZE',
    ftInteger, pdInput, 0, Znach_ZE_IK);
  (DataSet as TADOStoredProc).Parameters.CreateParameter
    ('@Hours_in_week_all_lesson', ftInteger, pdInput, 0, hWeekAllLes);
  (DataSet as TADOStoredProc).Parameters.CreateParameter
    ('@Hours_in_week_auditor_lesson', ftInteger, pdInput, 0, hWeekAuditorLess);
  (DataSet as TADOStoredProc).Parameters.CreateParameter('@min_holiday_week',
    ftInteger, pdInput, 0, MinHolidayWeek);
  (DataSet as TADOStoredProc).Parameters.CreateParameter('@max_holiday_week',
    ftInteger, pdInput, 0, MaxHolidayWeek);
  (DataSet as TADOStoredProc).Parameters.CreateParameter('@winter_holiday_week',
    ftInteger, pdInput, 0, WinterHolidayWeek);
  try
    DataSet.ExecProc;
    DataSet.Close;
  except
...
function TFgosController.SaveCompetence(OperationType: byte;
  ZnachCompetence, ShName: string; IKTypeCompetence, IDGos,
  IKCompetence: Integer): boolean;
var
  tempDS: TADOStoredProc;
begin
  tempDS := TGeneralController.Instance.GetNewADOStoredProc
    ('UpdateCompetence', false);
  tempDS.Parameters.CreateParameter('@i_type', ftWord, pdInput, 0,
    OperationType);
  tempDS.Parameters.CreateParameter('@ik_competence', ftInteger, pdInput, 0,
    IKCompetence);
  tempDS.Parameters.CreateParameter('@zhach_competence', ftString, pdInput,
    MAXINT, ZnachCompetence);
  tempDS.Parameters.CreateParameter('@short_Name', ftString, pdInput,
    10, ShName);
  tempDS.Parameters.CreateParameter('@ik_type_competence', ftInteger, pdInput,
    0, IKTypeCompetence);
...
function TFgosController.DeleteCompetence(IKCompetence, IKSpecCompetence
  : Integer): boolean;
var
  tempDS: TADOStoredProc;
begin
...
function TFgosController.SaveStructureCompetence(OperationType: byte;
  DescriptionContent: string; IKStructureCompetence, IKCompetence,
  IKLevelCompetence: Integer): boolean;
var
  tempDS: TADOStoredProc;
begin
  tempDS := TGeneralController.Instance.GetNewADOStoredProc
    ('UpdateStructureCompetence', false);
  tempDS.Parameters.CreateParameter('@i_type', ftWord, pdInput, 0,
    OperationType);
  tempDS.Parameters.CreateParameter('@ik_structure_competence', ftInteger,
    pdInput, 0, IKStructureCompetence);
  tempDS.Parameters.CreateParameter('@ik_competence', ftInteger, pdInput, 0,
    IKCompetence);
  tempDS.Parameters.CreateParameter('@ik_level_competence', ftInteger, pdInput,
    0, IKLevelCompetence);
  tempDS.Parameters.CreateParameter('@description_content', ftString, pdInput,
    MAXINT, DescriptionContent);
...
function TFgosController.DeleteStructureCompetence(IKStructureCompetence
  : Integer): boolean;
var
  tempDS: TADOStoredProc;
begin
...
function TFgosController.SaveLevelSformyr(OperationType: byte;
  Value_priznak: string; IKLevelSformyr, IKCompetence, IKTypeLevelSformyr,
  IKLevelCompetence: Integer): boolean;
var
  tempDS: TADOStoredProc;
begin
  tempDS := TGeneralController.Instance.GetNewADOStoredProc
    ('UpdateLevelSformyr', false);
  tempDS.Parameters.CreateParameter('@i_type', ftWord, pdInput, 0,
    OperationType);
  tempDS.Parameters.CreateParameter('@ik_level_sformyr', ftInteger, pdInput, 0,
    IKLevelSformyr);
  tempDS.Parameters.CreateParameter('@ik_competence', ftInteger, pdInput, 0,
    IKCompetence);
  tempDS.Parameters.CreateParameter('@ik_type_level_sformyr', ftInteger,
    pdInput, 0, IKTypeLevelSformyr);
...
begin
  ...
  tempDS.CommandText :=
    'select * from MW_Gos, Uch_pl where Uch_pl.ik_spec = MW_Gos.ik_spec and IDGos='
    + IntToStr(FgosIK);
  tempDS.Open;
  ...
  begin
    ShowMessage
      ('Невозможно удалить ФГОС. Для текущей специальности присутствуют учебные планы');
    Result := false;
  end
  else
  begin
