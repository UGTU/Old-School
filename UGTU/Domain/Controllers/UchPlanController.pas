...
type
  ...
  private
    ...
    function getValuesFromStringList(lpSource: PStringList; tableName: string; fieldIK:string; fieldName: string): string;
  protected
  ...
  public
    ...
    function CheckColumn(columnNumber: integer; value: string; DefaultKafedraIK: integer): boolean;
    ...
    function getAllFormEd(SourceDataSet: PDataSet; isShowFirst: boolean): Variant;

    {
        getAllDepartments - загружает все кафедры в SourceDataSet
        если isShowFirst, то возвращает значение атрибута keyField первой записи, иначе возвращает NULL
    }
    function getAllDepartments(SourceDataSet: PDataSet; isShowFirst: boolean): Variant;

    {
        getAllDisciplines - загружает все дисциплины в SourceDataSet
        если isShowFirst, то возвращает значение атрибута keyField первой записи, иначе возвращает NULL
    }
    function getAllDisciplines(SourceDataSet: PDataSet; isShowFirst: boolean): Variant;

    {
        getAllDisciplineCycles - загружает все циклы дисциплин в SourceDataSet
        если isShowFirst, то возвращает значение атрибута keyField первой записи, иначе возвращает NULL
    }
    function getAllDisciplineCycles(SourceDataSet: PDataSet; VidGos:integer; isShowFirst: boolean): Variant;

    {
        getAllDisciplineGroups - загружает все группы дисциплин в SourceDataSet
        если isShowFirst, то возвращает значение атрибута keyField первой записи, иначе возвращает NULL
    }
    function getAllDisciplineGroups(SourceDataSet: PDataSet; VidGos:integer; isShowFirst: boolean): Variant;

    {
        getAllDisciplineCyclesWithoutAll - загружает все циклы дисциплин, кроме записи <все>, в SourceDataSet
        если isShowFirst, то возвращает значение атрибута keyField первой записи, иначе возвращает NULL
    }
    function getAllDisciplineCyclesWithoutAll(SourceDataSet: PDataSet; VidGos: integer; isShowFirst: boolean): Variant;

    {
        getAllDisciplines - загружает все группы дисциплин, кроме записи <все>, в SourceDataSet
        если isShowFirst, то возвращает значение атрибута keyField первой записи, иначе возвращает NULL
    }
    function getAllDisciplineGroupsWithoutAll(SourceDataSet: PDataSet; VidGos: integer; isShowFirst: boolean): Variant;
    {
     ...
     function getAllPodGroups(SourceDataSet: PDataSet; isShowFirst: boolean): Variant;
    {
        getAllSpecializations - загружает все специализации специальности SpecIK в SourceDataSet
        если isShowFirst, то возвращает значение атрибута keyField первой записи, иначе возвращает NULL
    }
    function getAllSpecializations(SourceDataSet: PDataSet; SpecIK: integer; isShowFirst: boolean): Variant;

    {
        getAllYears - загружает все года в SourceDataSet
        если isShowFirst, то возвращает значение атрибута keyField первой записи, иначе возвращает NULL
    }
    function getAllYears(SourceDataSet: PDataSet; isShowFirst: boolean): Variant;
    ...
    function getColumnsNames(semestrStr: string; discTypeIK: integer): TStringList;
    ...
    procedure getColumnsValues(DiscInUchPlanIK: integer; var LectHour, LabHour, PractHour: integer;
                var ValuesColumn: TStringList; KafedraColumn: PStringList; isAlreadyOpen: boolean);
    ...
    function getSemestersInStr(SourceDataSet: PDataSet; UchPlanIK: integer; keyField: string): string;
    ...
    procedure LoadSelection(UchPlanIK, KafIK: integer; SemesterStr: string; ProgressBar: PProgressBar; SemLengthDataSet: PDataSet);
    ...
    function SaveSpecialization(SpclzIK, SpecIK: integer; CodeSpclz, NameSpclz, NameShortSpclz: string): boolean;
...
type
  ...
  private
    ...
    fNumber: integer;
  public
    constructor Create(AVidZanyatIK, ASemNumber, AKafIK, ANumber, ACount: integer);
  ...
  public
    ...
    procedure AddTaskRecord(AVidZanyatIK, ASemNumber, AKafIK, ANumber, ACount: integer);
    property WeekCount[AVidZanyatIK, ASemNumber, AKafIK: integer]: integer read GetWeekCount;
    property TaskCount[AVidZanyatIK, ASemNumber, AKafIK, ANumber: integer]: integer read GetTaskCount;
    property Items[Index: Integer]: TVidZanyatException read GetException write SetException; default;
  end;
  ...
  published
  public
    function GetNextSemKafNumber(AVidZanyatIK, ASemNumber, AKafIK: integer): integer;
...
begin
  case Request of
    0 : ;
    1 : if not Assigned(FUchPlanControllerInstance) then FUchPlanControllerInstance := CreateInstance;
    2 :
      begin
        if FUchPlanControllerInstance <> nil then
          FreeAndNil(FUchPlanControllerInstance);
      end
  else raise Exception.CreateFmt('Неизвестное значение параметра %d в методе AccessInstance контроллера UchPlanController', [Request]);
  end;
...
begin
  ...
  DataSet.Parameters.CreateParameter('@ik_uch_plan',ftInteger,pdInput, 0, NewUchPlanIK);
  ...
  DataSet.Parameters.CreateParameter('@ik_form_ed',ftInteger,pdInput, 0, NULL);
  DataSet.Parameters.CreateParameter('@ik_year_uch_pl',ftInteger,pdInput, 0, NULL);
  DataSet.Parameters.CreateParameter('@date_utv',ftDate,pdInput, 0, NULL);
  DataSet.Parameters.CreateParameter('@old_uch_plan',ftInteger,pdInput, 0, OldUchPlanIK);
  DataSet.ExecProc;
  ...
  tempDataSet.CommandText:= 'Select * From sv_disc Where ik_uch_plan = ' + IntToStr(OldUchPlanIK);
  tempDataSet.Open;
  ...
  begin
    ...
    DataSet.Parameters.ParamByName('@ik_disc').Value:= tempDataSet.FieldByName('ik_disc').Value;
    DataSet.Parameters.ParamByName('@ik_default_kaf').Value:= tempDataSet.FieldByName('ik_default_kaf').Value;
    DataSet.Parameters.ParamByName('@iHour_gos').Value:= tempDataSet.FieldByName('iHour_gos').Value;
    DataSet.Parameters.ParamByName('@iIndivid').Value:= tempDataSet.FieldByName('iIndivid').Value;
    DataSet.Parameters.ParamByName('@ik_ckl_disc').Value:= tempDataSet.FieldByName('ik_ckl_disc').Value;
    DataSet.Parameters.ParamByName('@ik_grp_disc').Value:= tempDataSet.FieldByName('ik_grp_disc').Value;
    DataSet.Parameters.ParamByName('@ik_pdgrp_disc').Value:= tempDataSet.FieldByName('ik_pdgrp_disc').Value;
    DataSet.Parameters.ParamByName('@Cname_ckl_disc_gos').Value:= tempDataSet.FieldByName('Cname_ckl_disc1').Value;
    DataSet.Parameters.ParamByName('@ViborGroup').Value:= tempDataSet.FieldByName('ViborGroup').Value;
    try
      DataSet.Open;
    ...
    DataSet.Parameters.ParamByName('@source_disc_uch_plan').Value:= tempDataSet.FieldByName('ik_disc_uch_plan').Value;
    try
      DataSet.ExecProc;
    except
      raise;
...
begin
  ...
  myStoredProc.Parameters.CreateParameter('@ik_disc_uch_plan', ftInteger, pdInput, 0, DiscInUchPlanIK);
  try
...
begin
  ...
  myStoredProc.Parameters.CreateParameter('@ik_uch_plan', ftInteger, pdInput, 0, UchPlanIK);
  try
    myStoredProc.ExecProc;
  except
    myStoredProc.Free;
...
function TUchPlanController.getAllFormEd(SourceDataSet: PDataSet; isShowFirst:
        boolean): Variant;
begin
  Result:= TGeneralController.Instance.getDataSetValues(SourceDataSet, 'Select * From Form_ed ORDER BY Cname_form_ed', 'ik_form_ed', isShowFirst, NULL);
end;

function TUchPlanController.getAllSpecializations(SourceDataSet: PDataSet; 
        SpecIK: integer; isShowFirst: boolean): Variant;
begin
  Result:= TGeneralController.Instance.getDataSetValues(SourceDataSet, 'Select * From spclz Where ik_spec = ' + IntToStr(SpecIK) + ' Order By cName_spclz', 'ik_spclz', isShowFirst, NULL);
end;

function TUchPlanController.getAllYears(SourceDataSet: PDataSet; isShowFirst: 
        boolean): Variant;
begin
  Result:= TGeneralController.Instance.getDataSetValues(SourceDataSet, 'Select * From Year_uch_pl ORDER BY year_value', 'ik_year_uch_pl', isShowFirst, NULL);
end;
...
var tempQuery: string;
begin
 tempQuery := 'Select Uch_pl.ik_uch_plan, Year_uch_pl.* From Uch_pl INNER JOIN Year_uch_pl ON Uch_pl.ik_year_utverzh = Year_uch_pl.ik_year_uch_pl Where (Uch_pl.ik_spec = ' + IntToStr(SpecIK) + ') and (Uch_pl.ik_form_ed = ' + VarToStr(FormEdIK) + ')';
...
begin
  Result:= TGeneralController.Instance.getDataSetValues(SourceDataSet, 'Select ckl_disc.IK_ckl_disc, RTRIM(ckl_disc.Cname_ckl_disc) as name_ckl_disc From ckl_disc where VidGos='+IntToStr(VidGos)+' or VidGos=3 ORDER BY Cname_ckl_disc', 'ik_ckl_disc', isShowFirst, NULL);
end;
...
begin
if (dmFgos.aspGetDiscZE.Active) then dmFgos.aspGetDiscZE.Close;
dmFgos.aspGetDiscZE.Parameters.ParamByName('@IDGos').Value:= FgosIK;
dmFgos.aspGetDiscZE.Open;
Result:=dmFgos.aspGetDiscZE;
end;

function TUchPlanController.getDiscExceptionsZE(FgosIK, DiscIK:  Integer): TDataSet;
begin
if (dmFgos.aspGetDiscExceptionZE.Active) then dmFgos.aspGetDiscExceptionZE.Close;
dmFgos.aspGetDiscExceptionZE.Parameters.ParamByName('@IDGos').Value:= FgosIK;
dmFgos.aspGetDiscExceptionZE.Parameters.ParamByName('@ik_disc').Value:= DiscIK;
...
begin
  ...
  DataSet.CommandText:= 'Select * From Spec_stud Where ik_spec = ' + IntToStr(SpecIK);
  DataSet.Open;
...
begin
  ...
  ' on d.iK_disc=s.ik_disc Where s.ik_disc_uch_plan= ' + IntToStr(IKDiscInUchPlan);
  DataSet.Open;
...
begin
  Result:= TGeneralController.Instance.getDataSetValues(SourceDataSet, 'Select ik_kaf, RTRIM(cname_kaf) as name_kaf, RTRIM(cshort_name_kaf) as short_name_kaf From kafedra Order By cname_kaf', 'ik_kaf', isShowFirst, 1);
end;

function TUchPlanController.getAllDisciplines(SourceDataSet: PDataSet;
  isShowFirst: boolean): Variant;
begin
  Result:= TGeneralController.Instance.getDataSetValues(SourceDataSet, 'Select discpln.iK_disc, RTRIM(discpln.cName_disc) as name_disc, discpln.ik_type_disc From discpln Order By cName_disc', 'iK_disc', isShowFirst, NULL);
end;
...
begin
  Result:= TGeneralController.Instance.getDataSetValues(SourceDataSet, 'Select ckl_disc.IK_ckl_disc, RTRIM(ckl_disc.Cname_ckl_disc) as name_ckl_disc, ckl_disc.Ccode_ckl_disc From ckl_disc Where (IK_ckl_disc != 11) and  (VidGos='+IntToStr(VidGos)+' or VidGos=3) Order By Cname_ckl_disc', 'ik_ckl_disc', isShowFirst, NULL);
end;

function TUchPlanController.getAllDisciplineGroupsWithoutAll(
  SourceDataSet: PDataSet; VidGos: integer; isShowFirst: boolean): Variant;
begin
  Result:= TGeneralController.Instance.getDataSetValues(SourceDataSet, 'Select grp_disc.IK_grp_disc, RTRIM(grp_disc.Cname_grp_disc) as name_grp_disc, grp_disc.Ccode_grp_disc, lVibor From grp_disc Where (IK_grp_disc != 9) and  (VidGos='+IntToStr(VidGos)+' or VidGos=3) Order By Cname_grp_disc', 'ik_grp_disc', isShowFirst, NULL);
end;
...
begin
  Result:= TGeneralController.Instance.getDataSetValues(SourceDataSet,
  'Select pdgrp_disc.iK_pdgrp_disc, RTRIM(pdgrp_disc.Cname_pdgrp_disc) as name_pdgrp_disc, pdgrp_disc.Ccode_pdgrp_disc From pdgrp_disc Order By Cname_pdgrp_disc', 'iK_pdgrp_disc', isShowFirst, NULL);
end;

function TUchPlanController.GetStringListFromStr(
...
function TUchPlanController.CheckSemesterString(
  sourceStr: string): boolean;
const allow: set of char = ['1','2','3','4','5','6','7','8','9','0'];
var
  i:integer;
  number: string;
begin
  ...
  if Length(sourceStr) > 0 then
    for i:= 1 to Length(sourceStr) do
    begin
      if ((not (sourceStr[i] in allow)) and (sourceStr[i] <> ' ') and (sourceStr[i] <> ',')) then
      begin
        Result:= false;
        exit;
      end;
      if (sourceStr[i] = ',') then
        begin
          if (Length(number) > 0) then
            if ((Length(number) > 2) or (StrToInt(number) > 12) or ((Length(number) = 1) and (StrToInt(number) = 0))) then
              begin
                Result:= false;
                exit;
              end;
...
function TUchPlanController.IsListEqual(first,
...
begin
  ...
  begin
    ...
    DataSet:= TADODataSet.Create(nil);
    DataSet.Connection:= dm.DBConnect;
    DataSet.CommandText:= 'Select * From ' + tableName + ' Where (' + tempStr + ')';
    DataSet.Open;
    tempStr:= '';
    ...
    if (Length(tempStr) > 0) then Delete(tempStr, Length(tempStr)-1, 1);
    DataSet.Close;
    DataSet.Free;
    Result:= tempStr;
  end
...
begin
  ...
  begin
    ...
    try
      tempDS.CommandText:= 'SELECT ik_vid_zanyat, ik_kaf, n_sem, week_count FROM Content_UchPl INNER JOIN UchPlan_WeekCount_Exception upwce ON Content_UchPl.ik_upContent = upwce.ik_upContent WHERE ik_disc_uch_plan = ' + IntToStr(DiscInUchPlanIK);
      tempDS.Open;
      while not tempDS.Eof do
      begin
        weekCountExceptionList.Add(TVidZanyatException.Create(tempDS.FieldByName('ik_vid_zanyat').AsInteger, tempDS.FieldByName('n_sem').AsInteger, tempDS.FieldByName('ik_kaf').AsInteger, 1, tempDS.FieldByName('week_count').AsInteger));
        tempDS.Next;
      end;
      tempDS.Close;

      tempDS.CommandText:= 'SELECT ik_upContent, tasks_count FROM Vid_zanyat_with_several_tasks WHERE ik_upContent in (SELECT ik_upContent FROM Content_UchPl WHERE ik_disc_uch_plan = ' + IntToStr(DiscInUchPlanIK) + ')';
      tempDS.Open;
      qContentUchPlan.First;
      while not qContentUchPlan.Eof do
      begin
        n:= semKafList.GetNextSemKafNumber(qContentUchPlan.FieldByName('ik_vid_zanyat').AsInteger, qContentUchPlan.FieldByName('n_sem').AsInteger, qContentUchPlan.FieldByName('ik_kaf').AsInteger);
        if tempDS.Locate('ik_upContent', qContentUchPlan.FieldByName('ik_upContent').AsInteger, [loPartialKey]) then
          vidZanyatTaskCountList.AddTaskRecord(qContentUchPlan.FieldByName('ik_vid_zanyat').AsInteger, qContentUchPlan.FieldByName('n_sem').AsInteger, qContentUchPlan.FieldByName('ik_kaf').AsInteger, n, tempDS.FieldByName('tasks_count').AsInteger);
...
function TUchPlanController.GetSemestersInStr(SourceDataSet: PDataSet; UchPlanIK: integer; keyField: string): string;
var
  n: integer;
begin
  ...
  (SourceDataSet^ as TADODataSet).CommandText:= 'Select * From sem_uch_pl Where ik_uch_plan = ' + IntToStr(UchPlanIK) + ' Order By n_sem';
  SourceDataSet^.Open;
  ...
  if (SourceDataSet^.RecordCount > 0) then
    begin
      ...
      begin
        ...
        if (n = 0) then Result:= '(' + keyField + '= ' + IntToStr(semesters[n].number) + ')'
        else Result:= Result + ' or (' + keyField + ' = ' + IntToStr(semesters[n].number) + ')';
        inc(n);
        SourceDataSet^.Next;
      end;
    end
  else Result:= keyField + ' = -1';
end;

function TUchPlanController.getColumnsNames(semestrStr: string; discTypeIK: integer): TStringList;
var
  n: integer;
  DataSet: TADODataSet;
begin
  DataSet:= TADODataSet.Create(nil);
  DataSet.Connection:= dm.DBConnect;
  (DataSet as TADODataSet).CommandText:= 'SELECT count(ik_column) as NoAudRowCount FROM Uch_plan_columns WHERE (i_type_column = 3) and (ik_type_disc = ' + IntToStr(discTypeIK) + ')';
  DataSet.Open;
  NoAudRowCount:= DataSet.FieldByName('NoAudRowCount').AsInteger;
  DataSet.Close;
  case discTypeIK of
  1: (DataSet as TADODataSet).CommandText:= 'SELECT * FROM Uch_plan_columns WHERE (ik_type_disc = 1) and (((i_type_column IN (3, 4, 5, 6)) or ((i_type_column = 11) and (' + semestrStr + ')))) ORDER BY ik_column';
  else (DataSet as TADODataSet).CommandText:= 'SELECT * FROM Uch_plan_columns WHERE ik_type_disc = ' + IntToStr(discTypeIK);
  end;
  DataSet.Open;
  Result:= TStringList.Create;
  case discTypeIK of
  1: SetLength(arColumns, DataSet.RecordCount - 3);  // минус лекции, практ. и лаб.
...
begin
  ...
  begin
    ...
    dm.qContentUchPlan.SQL.Add('Select * From Content_UchPl Where ik_disc_uch_plan = ' + IntToStr(DiscInUchPlanIK));
            ...
            begin
              ...
              begin
                ...
                begin
                  ...
                  tempStr:= tempStr + dm.qContentUchPlan.FieldByName('n_sem').AsString + ', ';
                end;
                dm.qContentUchPlan.Next;
              end;
              if (Length(tempStr) > 0) then Delete(tempStr, Length(tempStr)-1, 2);
              ValuesColumn.Add(tempStr);
              if (KafedraColumn <> nil) then
                KafedraColumn^.Add(GetValuesFromStringList(@tempSL, 'kafedra','ik_kaf','cshort_name_kaf'));
            end;
          11:
            begin
              for g:= 0 to Length(semesters)-1 do
                    ...
                    begin
                      ...
                      begin
                        ...
                        begin
                          ...
                          else semesters[g].auditorHour[i]:= semesters[g].auditorHour[i] + dm.qContentUchPlan.FieldByName('i_hour_per_week').AsInteger;
                          if (dm.qContentUchPlan.FieldByName('ik_kaf').Value <> NULL) then
                          begin
                            if (tempSL.IndexOf(dm.qContentUchPlan.FieldByName('ik_kaf').AsString) = -1) then
                              tempSL.Add(dm.qContentUchPlan.FieldByName('ik_kaf').AsString);

                            if (i = 0) then LectHour:= LectHour + weekCountExceptionList.WeekCount[auditorVZ[i], semesters[g].number, dm.qContentUchPlan.FieldByName('ik_kaf').AsInteger] * dm.qContentUchPlan.FieldByName('i_hour_per_week').AsInteger;
                            if (i = 1) then LabHour:= LabHour + weekCountExceptionList.WeekCount[auditorVZ[i], semesters[g].number, dm.qContentUchPlan.FieldByName('ik_kaf').AsInteger] * dm.qContentUchPlan.FieldByName('i_hour_per_week').AsInteger;
                            if (i = 2) then PractHour:= PractHour + weekCountExceptionList.WeekCount[auditorVZ[i], semesters[g].number, dm.qContentUchPlan.FieldByName('ik_kaf').AsInteger] * dm.qContentUchPlan.FieldByName('i_hour_per_week').AsInteger;
                          end;
                        end;
                      end;
                      dm.qContentUchPlan.Next;
              ...
              {      LectHour:= LectHour + semesters[g].weekCount * semesters[g].auditorHour[0];
                    ...
                    secTempStr:= '';
                    if semesters[g].auditorHour[0] = -1 then secTempStr:= '??\'
                    else secTempStr:= IntToStr(semesters[g].auditorHour[0]) + '\';
                    if semesters[g].auditorHour[1] = -1 then secTempStr:= secTempStr + '??\'
                    else secTempStr:= secTempStr + IntToStr(semesters[g].auditorHour[1]) + '\';
                    if semesters[g].auditorHour[2] = -1 then secTempStr:= secTempStr + '??'
                    else secTempStr:= secTempStr + IntToStr(semesters[g].auditorHour[2]);
  ...
  try
    ...
    begin
      ...
      for i := 0 to weekCountExceptionList.Count - 1 do
        if dm.qContentUchPlan.Locate('ik_vid_zanyat; n_sem; ik_kaf', VarArrayOf([weekCountExceptionList[i].VidZanyatIK, weekCountExceptionList[i].SemNumber, weekCountExceptionList[i].KafIK]), [loPartialKey]) then
        begin
          tempDS.Insert;
          tempDS.FieldByName('ik_upContent').AsInteger:= dm.qContentUchPlan.FieldByName('ik_upContent').AsInteger;
          tempDS.FieldByName('week_count').AsInteger:= weekCountExceptionList[i].Count;
          tempDS.Post;
        end;
    end;
    tempDS.UpdateBatch();
    ...
    tempDS.CommandText:= 'SELECT * FROM Vid_zanyat_with_several_tasks WHERE ik_upContent in (SELECT ik_upContent FROM Content_UchPl WHERE ik_disc_uch_plan = ' + IntToStr(DiscInUchPlanIK) + ')';
    tempDS.Open;
    while not tempDS.Eof do tempDS.Delete;
    ...
    while not dm.qContentUchPlan.Eof do
    begin
      n:= semKafList.GetNextSemKafNumber(dm.qContentUchPlan.FieldByName('ik_vid_zanyat').AsInteger, dm.qContentUchPlan.FieldByName('n_sem').AsInteger, dm.qContentUchPlan.FieldByName('ik_kaf').AsInteger);
      n:= vidZanyatTaskCountList.TaskCount[dm.qContentUchPlan.FieldByName('ik_vid_zanyat').AsInteger, dm.qContentUchPlan.FieldByName('n_sem').AsInteger, dm.qContentUchPlan.FieldByName('ik_kaf').AsInteger, n];
      if n > 1 then
      begin
        tempDS.Insert;
        tempDS.FieldByName('ik_upContent').AsInteger:= dm.qContentUchPlan.FieldByName('ik_upContent').AsInteger;
    ...
    begin
      ...
      tempDS.FieldByName('ik_structure_competence').Value := aStrCompetenceList[i];
      tempDS.Post;
    end;
    tempDS.UpdateBatch();
    if tempDS.Active then tempDS.Close;
     ...
     tempDS.CommandText := 'select * from sv_disc_relation where depends_on_ik = '+IntToStr(DiscInUchPlanIK);
    tempDS.Open;
    tempDS.First;
    while not tempDS.Eof do
    begin
...
begin
  DataSet:= TADODataSet.Create(nil);
  DataSet.Connection:= dm.DBConnect;
  DataSet.CommandText:= 'Select * From sv_disc Where (ik_uch_plan = ' + IntToStr(UchPlanIK) + ') and (ik_disc = ' + IntToStr(DiscIK) + ')';
  DataSet.Open;
  ...
  begin
    //Result:= -1;
    raise Exception.Create('В базе данных отсутствует запись со значением атрибутов ik_uch_plan = ' + IntToStr(UchPlanIK) + 'и ik_disc = ' + IntToStr(DiscIK));
  end;
  DataSet.Close;
  DataSet.Free;
end;

function TUchPlanController.CheckColumn(columnNumber: integer; value: string; DefaultKafedraIK: integer): boolean;
var
  i, g, n:integer;
  tempStr: string;
...
begin
  ...
  try
    ...
    3:
      begin
        tempStr:= '';
        if (not CheckSemesterString(value)) then Exit;
        IKList:= GetStringListFromStr(value);  //получаем номера семестров, которые только что ввели
        ...
        dm.qContentUchPlan.First;      //прочитаем семестры которые есть в таблице по данному виду занятий
        while (not dm.qContentUchPlan.Eof) do
        begin
        ...
        begin
          ...
          begin
            ...
            begin
              if dm.qContentUchPlan.Locate('iK_vid_zanyat; n_sem', VarArrayOf([arColumns[columnNumber].vidZanyatIK, TempIKList[0]]), [loPartialKey]) then
              begin
                dm.qContentUchPlan.Edit;
    ...
    11:
      begin
        ...
        if (not IsAuditorCorrect(value)) then Exit;
        ...
        for g:= 0 to Length(semesters)-1 do
          ...
          begin
              tempStr:= '';
              for i:= 0 to 2 do
                if dm.qContentUchPlan.Locate('n_sem; iK_vid_zanyat', VarArrayOf([semesters[g].number, auditorVZ[i]]), [loPartialKey]) then
                begin
                  if dm.qContentUchPlan.FieldByName('i_hour_per_week').AsInteger >= 10 then
                    tempStr:= tempStr + dm.qContentUchPlan.FieldByName('i_hour_per_week').AsString + '\'
                  else tempStr:= tempStr + '0' + dm.qContentUchPlan.FieldByName('i_hour_per_week').AsString + '\';
                end
                else tempStr:= tempStr + '00\';
              if (Length(tempStr) > 0) then Delete(tempStr, Length(tempStr), 1);
    ...
    if Assigned(TempIKList) then FreeAndNil(TempIKList);
    raise;
  end;
  Result:=true;
end;
...
begin
  tempDS:= TGeneralController.Instance.GetNewADODataSet(false);
  try
    tempDS.CommandText:= 'SELECT CONVERT(float, ISNULL(constant_value, 5)) as Cons_percent FROM Nagruzka_constant WHERE ik_constant = 1';
    tempDS.Open;
...
function TUchPlanController.getColumnParamKey(
  columnNumber: integer): integer;
begin
...
function TUchPlanController.SaveAuditorVZ(Semester, DefaultKaf: integer): boolean;
var
  i: byte;
...
try
  ...
  begin
    ...
    isLastDelete:= false;
    for i:= 0 to 2 do
      if (dm.qContentUchPlan.FieldByName('ik_vid_zanyat').AsInteger = auditorVZ[i]) then
      begin
        if not dm.adsAuditorVZ.Locate('ik_vid_zanyat; ik_kaf', VarArrayOf([auditorVZ[i], dm.qContentUchPlan.FieldByName('ik_kaf').AsInteger]), [loPartialKey]) then
        begin
          isLastDelete:= true;
          dm.qContentUchPlan.Delete;
        end
        else isLastDelete:= false;
      end
      else isLastDelete:= false;
    if not isLastDelete then dm.qContentUchPlan.Next;
  end;
  ...
  begin
    ...
    begin
      MessageBox(FMessageHandle, 'Необходимо указать количество часов в неделю по всем видам занятий!', 'Проверка корректности значений', MB_ICONWARNING);
      Result:= false;
      exit;
    end;
    ...
    begin
      MessageBox(FMessageHandle, 'Необходимо указать количество недель по всем видам занятий для каждой кафедры!', 'Проверка корректности значений', MB_ICONWARNING);
      Result:= false;
      exit;
    end;
    ...
    begin
      if (dm.adsAuditorVZ.FieldByName('week_count').AsInteger > GetDefaultSemesterWeekCount(Semester)) then
      begin
        MessageBox(FMessageHandle, 'Значение в столбце "Количество недель" не может быть больше, чем количество недель, указанное в параметрах учебного плана для данного семестра!', 'Проверка корректности значений', MB_ICONWARNING);
        Result:= false;
        exit;
      end;
    end;
    ...
    if dm.qContentUchPlan.Locate('ik_vid_zanyat; ik_kaf; n_sem', VarArrayOf([dm.adsAuditorVZ.FieldByName('ik_vid_zanyat').asInteger, dm.adsAuditorVZ.FieldByName('ik_kaf').asInteger, Semester]), [loPartialKey]) then
    begin
      dm.qContentUchPlan.Edit;
      dm.qContentUchPlan.FieldByName('i_hour_per_week').Value:= dm.adsAuditorVZ.FieldByName('i_hour_per_week').AsInteger;
    end
    else
    begin
      dm.qContentUchPlan.Insert;
      dm.qContentUchPlan.FieldByName('n_sem').Value:= Semester;
      dm.qContentUchPlan.FieldByName('ik_vid_zanyat').Value:= dm.adsAuditorVZ.FieldByName('ik_vid_zanyat').Value;
      dm.qContentUchPlan.FieldByName('i_hour_per_week').Value:= dm.adsAuditorVZ.FieldByName('i_hour_per_week').Value;
      dm.qContentUchPlan.FieldByName('ik_kaf').Value:= dm.adsAuditorVZ.FieldByName('ik_kaf').Value;
    end;
    dm.qContentUchPlan.FieldByName('n_module').Value:= dm.adsAuditorVZ.FieldByName('n_module').Value;
    dm.qContentUchPlan.FieldByName('i_balls').Value:= dm.adsAuditorVZ.FieldByName('i_balls').Value;

    dm.qContentUchPlan.Post;
    if (dm.adsAuditorVZ.FieldByName('week_count').AsInteger <> GetDefaultSemesterWeekCount(Semester)) then
      weekCountExceptionList.Add(TVidZanyatException.Create(dm.adsAuditorVZ.FieldByName('ik_vid_zanyat').AsInteger, Semester, dm.adsAuditorVZ.FieldByName('ik_kaf').AsInteger, 1, dm.adsAuditorVZ.FieldByName('week_count').AsInteger));
    dm.adsAuditorVZ.Next;
  end;
  Result:= true;
except
...
function TUchPlanController.LoadAuditorVZ(DiscUchPlanIK, Semester: integer): TDataSet;
var
  i: integer;
  tempStr: string;
begin
  ...
  begin
    ...
    if i <> 2 then tempStr:= tempStr + ', '
    else tempStr:= tempStr + ')';
  end;
  if dm.adsVidZanyat.Active then dm.adsVidZanyat.Close;
  dm.adsVidZanyat.CommandText:= 'SELECT * FROM vid_zaniat WHERE ik_vid_zanyat in ' + tempStr;
  if dm.adsAuditorVZ.Active then dm.adsAuditorVZ.Close;
  dm.adsAuditorVZ.CommandText:= 'SELECT ik_vid_zanyat, ik_kaf, i_hour_per_week, week_count, n_module, i_balls FROM Content_UchPl LEFT JOIN UchPlan_WeekCount_Exception upwce ON Content_UchPl.ik_upContent = upwce.ik_upContent WHERE (ik_disc_uch_plan = ' + IntToStr(DiscUchPlanIK) + ') and (ik_vid_zanyat in ' + tempStr + ') and (n_sem = ' + IntToStr(Semester) + ')';
  ...
  begin
    for i:= 0 to 2 do
    begin
      if (dm.qContentUchPlan.FieldByName('n_sem').AsInteger = Semester) and (dm.qContentUchPlan.FieldByName('ik_vid_zanyat').AsInteger = auditorVZ[i]) then
      begin
       dm.adsAuditorVZ.Insert;
       dm.adsAuditorVZ.FieldByName('ik_vid_zanyat').Value:= auditorVZ[i];
       dm.adsAuditorVZ.FieldByName('ik_kaf').Value:= dm.qContentUchPlan.FieldByName('ik_kaf').AsInteger;
       dm.adsAuditorVZ.FieldByName('i_hour_per_week').Value:= dm.qContentUchPlan.FieldByName('i_hour_per_week').AsInteger;
       dm.adsAuditorVZ.FieldByName('n_module').Value:= dm.qContentUchPlan.FieldByName('n_module').Value;
       dm.adsAuditorVZ.FieldByName('i_balls').Value:= dm.qContentUchPlan.FieldByName('i_balls').Value;

       if Assigned(weekCountExceptionList) then
         dm.adsAuditorVZ.FieldByName('week_count').Value:= weekCountExceptionList.WeekCount[auditorVZ[i], Semester, dm.qContentUchPlan.FieldByName('ik_kaf').AsInteger];
       dm.adsAuditorVZ.Post;
      end;
    end;
    dm.qContentUchPlan.Next;
...
function TUchPlanController.LoadControlVZ(DiscInUchPlanIK,
  VidZanyatIK: integer): TDataSet;
var
  semKafList: TSemKafList;
begin
  semKafList:= TSemKafList.Create;
try
  if (dm.adsContentVZ.Active) then dm.adsContentVZ.Close;
  if (dm.adsKafedra.Active) then dm.adsKafedra.Close;
  if (dm.adsSemester.Active) then dm.adsSemester.Close;

  dm.adsKafedra.Open;
  dm.adsSemester.Open;
  dm.adsContentVZ.CommandText:= 'Select ik_kaf, n_sem, tasks_count, n_module, i_balls From Content_UchPl LEFT JOIN Vid_zanyat_with_several_tasks vzst ON Content_UchPl.ik_upContent = vzst.ik_upContent Where ((ik_disc_uch_plan = ' + IntToStr(DiscInUchPlanIK) + ') and (ik_vid_zanyat = ' + IntToStr(VidZanyatIK) + '))';
  ...
  begin
    if (dm.qContentUchPlan.FieldByName('iK_vid_zanyat').AsInteger = VidZanyatIK) then
      begin
        ...
        dm.adsContentVZ.FieldByName('n_sem').Value:= dm.qContentUchPlan.FieldByName('n_sem').Value;
        dm.adsContentVZ.FieldByName('ik_kaf').Value:= dm.qContentUchPlan.FieldByName('ik_kaf').Value;
        dm.adsContentVZ.FieldByName('n_module').Value:= dm.qContentUchPlan.FieldByName('n_module').Value;
        dm.adsContentVZ.FieldByName('i_balls').Value:= dm.qContentUchPlan.FieldByName('i_balls').Value;

        if Assigned(vidZanyatTaskCountList) then
          dm.adsContentVZ.FieldByName('tasks_count').Value:= vidZanyatTaskCountList.TaskCount[VidZanyatIK, dm.qContentUchPlan.FieldByName('n_sem').AsInteger, dm.qContentUchPlan.FieldByName('ik_kaf').AsInteger, semKafList.GetNextSemKafNumber(VidZanyatIK, dm.qContentUchPlan.FieldByName('n_sem').AsInteger, dm.qContentUchPlan.FieldByName('ik_kaf').AsInteger)];
    ...
    begin
      ...
      if IKList.Count > 0 then
        ...
        begin
          ...
          dm.qContentUchPlan.FieldByName('iK_vid_zanyat').Value:= vidZanyatIK;
          dm.qContentUchPlan.FieldByName('n_sem').Value:= IKList[i];
          if ModuleIKList[i]<>'null' then begin
          if dm.adsContentVZ.Locate('n_sem; n_module',  VarArrayOf([IKList[i],ModuleIKList[i]]), [loPartialKey]) then
          begin
            dm.qContentUchPlan.FieldByName('ik_kaf').Value:= dm.adsContentVZ.FieldByName('ik_kaf').Value;
            dm.qContentUchPlan.FieldByName('n_module').Value:= dm.adsContentVZ.FieldByName('n_module').Value;
            dm.qContentUchPlan.FieldByName('i_balls').Value:= dm.adsContentVZ.FieldByName('i_balls').Value;
          ...
          else if dm.adsContentVZ.Locate('n_sem',  IKList[i], [loPartialKey]) then
          begin
            dm.qContentUchPlan.FieldByName('ik_kaf').Value:= dm.adsContentVZ.FieldByName('ik_kaf').Value;
            dm.qContentUchPlan.FieldByName('n_module').Value:= dm.adsContentVZ.FieldByName('n_module').Value;
            dm.qContentUchPlan.FieldByName('i_balls').Value:= dm.adsContentVZ.FieldByName('i_balls').Value;
  ...
  begin
    ...
    begin
        ...
        begin
          ...
          while not dm.adsContentVZ.Eof do
          begin
            if (dm.adsContentVZ.FieldByName('n_sem').AsInteger = dm.qContentUchPlan.FieldByName('n_sem').AsInteger) and
            (TempIKList.IndexOf(IntToStr(dm.adsContentVZ.RecNo)) < 0) then
            begin
              dm.qContentUchPlan.Edit;
              dm.qContentUchPlan.FieldByName('ik_kaf').AsInteger:= dm.adsContentVZ.FieldByName('ik_kaf').AsInteger;
              dm.qContentUchPlan.FieldByName('n_module').Value:= dm.adsContentVZ.FieldByName('n_module').Value;
              dm.qContentUchPlan.FieldByName('i_balls').Value:= dm.adsContentVZ.FieldByName('i_balls').Value;
...
//  dm.adsContentVZ.EnableControls;
  ...
  if Assigned(IKList) then FreeAndNil(IKList);
  if Assigned(TempIKList) then FreeAndNil(TempIKList);
  if Assigned(semKafList) then FreeAndNil(semKafList);  
end;
end;

procedure TUchPlanController.LoadSelection(UchPlanIK, KafIK: integer; SemesterStr: string; ProgressBar: PProgressBar; SemLengthDataSet: PDataSet);
var
  Disp: IDispatch;
  IExcelApp: ExcelApplication;
  RangeE: ExcelRange;
  CycleRow, GroupRow,  RowInCycle, RowInGroup: integer;
  CycleDataSet, GroupDataSet, DiscDataSet, SemDataSet, CompetDataSet, FgosDataSet: TADODataSet;
  lect, lab, pract, i, j, k, year_count, sem_ind, no_aud, ZE, cklZE, grpZE, allZE :integer;
  slColumnValues, slKafedraList:TStringList;
  tempSL, tempCSL: TStringList;
  tempStr: string;
const
  rimChisl: array[0..5] of string = ('I','II','III','IV','V','VI');
  SemContent: array[0..2] of string = ('Лекции','Лабораторные','Практические');
  semCols:array[1..52] of string = ('T', 'U', 'V', 'W', 'X','Y','Z','AA', 'AB', 'AC', 'AD','AE', 'AF', 'AG', 'AH', 'AI', 'AJ','AK', 'AL', 'AM', 'AN', 'AO', 'AP', 'AQ', 'AR', 'AS', 'AT', 'AU', 'AV', 'AW', 'AX','AY','AZ', 'BA', 'BB', 'BC', 'BD','BE', 'BF', 'BG', 'BH', 'BI', 'BJ','BK', 'BL', 'BM', 'BN', 'BO', 'BP', 'BQ', 'BR','BS');
...
try
  ...
  if (KafIK = 1) then
    DiscDataSet.CommandText:= 'Select count(*) as DiscCount From sv_disc Where ik_uch_plan = ' + IntToStr(UchPlanIK)
  else DiscDataSet.CommandText:= 'Select count(*) as DiscCount From sv_disc Where (ik_disc_uch_plan in (Select ik_disc_uch_plan From Content_UchPl Where ik_kaf = ' + IntToStr(KafIK) + ')) and (ik_uch_plan = ' + IntToStr(UchPlanIK) + ')';
  DiscDataSet.Open;
  if (DiscDataSet.FieldByName('DiscCount').AsInteger = 0) then
  begin
    MessageBox(MessageHandle, 'В выбранном учебном плане нет дисциплин, удовлетворяющих заданному условию!','Экспорт данных в Microsoft Excel',MB_ICONINFORMATION);
    DiscDataSet.Close;
    exit;
  end;
  ...
  RangeE:= IExcelApp.Range['A1', semCols[Length(arColumns) - (NoAudRowCount - 1)] + '1'];
  ...
  RangeE:= IExcelApp.Range['A2', semCols[Length(arColumns) - (NoAudRowCount - 1)] + '2'];
  RangeE.MergeCells:= true;
  RangeE.Value2:= 'УЧЕБНЫЙ РАБОЧИЙ ПЛАН';
  RangeE.RowHeight:= IExcelApp.CentimetersToPoints(1.5, 0);

  DiscDataSet.CommandText:= 'Select * From Uch_pl up INNER JOIN Spec_stud ss ON up.ik_spec = ss.ik_spec left JOIN spclz ON up.ik_spclz = spclz.ik_spclz ' +
      ...
      'on (rsf.ik_spec = ss.ik_spec)and(rsf.Ik_form_ed = up.Ik_form_ed) Where ik_uch_plan = ' + IntToStr(UchPlanIK);
  DiscDataSet.Open;
  year_count :=  DiscDataSet.FieldByName('YearObuch').asInteger;
  FgosDataSet.CommandText := 'select znach_ZE from Znachenie_ZE where ik_znach_ZE=(select ik_znach_ZE from MW_Gos where ik_spec='+DiscDataSet.FieldByName('ik_spec').AsString+')';
  FgosDataSet.Open;
  ...
  RangeE:= IExcelApp.Range['A3', semCols[Length(arColumns) - (NoAudRowCount - 1)] + '3'];
  RangeE.MergeCells:= true;

  tempStr := 'Специальность: ' + Trim(DiscDataSet.FieldByName('Sh_spec').AsString) + ' ' + Trim(DiscDataSet.FieldByName('Cname_spec').AsString);
  if DiscDataSet.FieldByName('ik_spclz').AsInteger<>0 then
  case DiscDataSet.FieldByName('ik_direction').AsInteger of
  1: tempStr:=tempStr + ', профиль: ';
  2: tempStr:=tempStr + ', специализация: ';
  3: tempStr:=tempStr + ', программа: ';
  4: tempStr:=tempStr + ', специализация: ';
  end;

  RangeE.Value2 := tempStr + Trim(DiscDataSet.FieldByName('cName_spclz').AsString) + ', форма обучения: ' + Trim(DiscDataSet.FieldByName('Cname_form_ed').AsString) + ', год утверждения: ' + Trim(DiscDataSet.FieldByName('year_value').AsString);
  RangeE.RowHeight:= IExcelApp.CentimetersToPoints(1.5, 0);
  DiscDataSet.Close;

  CycleRow:= 4;

  RangeE:= IExcelApp.Range['A' + IntToStr(CycleRow), 'A' + IntToStr(CycleRow + 4)];
  RangeE.MergeCells:= true;
  RangeE.Value2:= 'Закреплено за кафедрой';
  RangeE.Orientation:= 90;

  RangeE:= IExcelApp.Range['B' + IntToStr(CycleRow), 'B' + IntToStr(CycleRow + 4)];
  RangeE.MergeCells:= true;
  RangeE.Value2:= 'Индекс';
  RangeE.Orientation:= 90;

  RangeE:= IExcelApp.Range['C' + IntToStr(CycleRow), 'C' + IntToStr(CycleRow + 4)];
  RangeE.MergeCells:= true;
  RangeE.Value2:= 'Название циклов и дисциплин';

  RangeE:= IExcelApp.Range['D' + IntToStr(CycleRow), 'K' + IntToStr(CycleRow)];
  RangeE.MergeCells:= true;
  RangeE.Value2:= 'Распределение по семестрам';

  RangeE:= IExcelApp.Range['D' + IntToStr(CycleRow + 1), 'D' + IntToStr(CycleRow + 4)];
  RangeE.MergeCells:= true;
  RangeE.Value2:= 'Экзамены';
  RangeE.Orientation:= 90;

 RangeE:= IExcelApp.Range['E' + IntToStr(CycleRow + 1), 'E' + IntToStr(CycleRow + 4)];
  RangeE.MergeCells:= true;
  RangeE.Value2:= 'Зачеты';
  RangeE.Orientation:= 90;

  RangeE:= IExcelApp.Range['F' + IntToStr(CycleRow + 1), 'F' + IntToStr(CycleRow + 4)];
  RangeE.MergeCells:= true;
  RangeE.Value2:= 'Курсовой проект';
  RangeE.Orientation:= 90;

   RangeE:= IExcelApp.Range['G' + IntToStr(CycleRow + 1), 'G' + IntToStr(CycleRow + 4)];
  RangeE.MergeCells:= true;
  RangeE.Value2:= 'Курсовая работа';
  RangeE.Orientation:= 90;

  RangeE:= IExcelApp.Range['H' + IntToStr(CycleRow+1), 'H' + IntToStr(CycleRow + 4)];
  RangeE.MergeCells:= true;
  RangeE.Value2:= 'РГР';
  RangeE.Orientation:= 90;

  RangeE:= IExcelApp.Range['I' + IntToStr(CycleRow+1), 'I' + IntToStr(CycleRow + 4)];
  RangeE.MergeCells:= true;
  RangeE.Value2:= 'Реферат';
  RangeE.Orientation:= 90;

  RangeE:= IExcelApp.Range['J' + IntToStr(CycleRow+1), 'J' + IntToStr(CycleRow + 4)];
  RangeE.MergeCells:= true;
  RangeE.Value2:= 'Контрольная работа';
  RangeE.Orientation:= 90;

  RangeE:= IExcelApp.Range['K' + IntToStr(CycleRow+1), 'K' + IntToStr(CycleRow + 4)];
  RangeE.MergeCells:= true;
  RangeE.Value2:= 'Аттестации';
  RangeE.Orientation:= 90;

  RangeE:= IExcelApp.Range['L' + IntToStr(CycleRow), 'L' + IntToStr(CycleRow + 4)];

  RangeE.MergeCells:= true;
  RangeE.Value2:= 'Всего (зач.единиц)';
  RangeE.Orientation:= 90;

  RangeE:= IExcelApp.Range['M' + IntToStr(CycleRow), 'S' + IntToStr(CycleRow)];
  RangeE.MergeCells:= true;
  RangeE.Value2:= 'Обязательная работа студента';

  RangeE:= IExcelApp.Range['M' + IntToStr(CycleRow+1), 'M' + IntToStr(CycleRow + 4)];
  RangeE.MergeCells:= true;
  RangeE.Value2:= 'Всего (часов)';
  RangeE.Orientation:= 90;

  RangeE:= IExcelApp.Range['N' + IntToStr(CycleRow+1), 'N' + IntToStr(CycleRow + 4)];
  RangeE.MergeCells:= true;
  RangeE.Value2:= 'Всего аудиторных';
  RangeE.Orientation:= 90;

  RangeE:= IExcelApp.Range['O' + IntToStr(CycleRow+1), 'Q' + IntToStr(CycleRow + 1)];
  RangeE.MergeCells:= true;
  RangeE.Value2:= 'Из них';

  RangeE:= IExcelApp.Range['O' + IntToStr(CycleRow+2), 'O' + IntToStr(CycleRow + 4)];
  RangeE.MergeCells:= true;
  RangeE.Value2:= 'Лекции';
  RangeE.Orientation:= 90;

  RangeE:= IExcelApp.Range['P' + IntToStr(CycleRow+2), 'P' + IntToStr(CycleRow + 4)];
  RangeE.MergeCells:= true;
  RangeE.Value2:= 'Лабораторные';
  RangeE.Orientation:= 90;

  RangeE:= IExcelApp.Range['Q' + IntToStr(CycleRow+2), 'Q' + IntToStr(CycleRow + 4)];
  RangeE.MergeCells:= true;
  RangeE.Value2:= 'Практические';
  RangeE.Orientation:= 90;

  RangeE:= IExcelApp.Range['R' + IntToStr(CycleRow+1), 'R' + IntToStr(CycleRow + 4)];
  RangeE.MergeCells:= true;
  RangeE.Value2:= 'Индивидуальные занятия';
  RangeE.Orientation:= 90;

  RangeE:= IExcelApp.Range['S' + IntToStr(CycleRow+1), 'S' + IntToStr(CycleRow + 4)];
  RangeE.MergeCells:= true;
  RangeE.Value2:= 'Самост. работа';
  RangeE.Orientation:= 90;


  RangeE:= IExcelApp.Range[semCols[1] + IntToStr(CycleRow), semCols[6*year_count] + IntToStr(CycleRow)];
  RangeE.MergeCells:= true;
  RangeE.Value2:= 'Распределение аудиторных занятий по курсам и семестрам (зачетных единиц)';

  SemDataSet.CommandText := 'Select * from sem_uch_pl where ik_uch_plan='+ IntToStr(UchPlanIK);
  SemDataSet.Open;
...
'(select ik_disc_uch_plan from sv_disc where ik_uch_plan = '+IntToStr(UchPlanIK)+')';

CompetDataSet.Open;

  for i := 0 to year_count-1 do
  begin
    RangeE:= IExcelApp.Range[semCols[6*i+1] + IntToStr(CycleRow+1), semCols[6*i+6] + IntToStr(CycleRow + 1)];
    RangeE.MergeCells:= true;
    RangeE.Value2:= rimChisl[i] + ' курс';
    for j := 0 to 1 do
    begin
      RangeE:= IExcelApp.Range[semCols[6*i+j*3+1] + IntToStr(CycleRow+2), semCols[6*i+j*3+3] + IntToStr(CycleRow + 2)];
      RangeE.MergeCells:= true;
      RangeE.Value2:= IntToStr(j+1) + ' семестр';
      SemDataSet.Filter := 'n_sem='+IntToStr(2*i+j+1);
      SemDataSet.Filtered := true;

      RangeE:= IExcelApp.Range[semCols[6*i+j*3+1] + IntToStr(CycleRow+3), semCols[6*i+j*3+1] + IntToStr(CycleRow + 3)];
      RangeE.MergeCells:= true;
      RangeE.Value2:= SemDataSet.FieldByName('kol_ned').AsString;
      RangeE.Font.ColorIndex := 3;

      RangeE:= IExcelApp.Range[semCols[6*i+j*3+2] + IntToStr(CycleRow+3), semCols[6*i+j*3+3] + IntToStr(CycleRow + 3)];
      RangeE.MergeCells:= true;
      ...
      for k := 0 to 2 do
      begin
        RangeE:= IExcelApp.Range[semCols[6*i+j*3+k+1] + IntToStr(CycleRow+4), semCols[6*i+j*3+k+1] + IntToStr(CycleRow + 4)];
        RangeE.MergeCells:= true;
  ...
  RangeE:= IExcelApp.Range[semCols[6*year_count+1] + IntToStr(CycleRow), semCols[6*year_count+1] + IntToStr(CycleRow + 4)];
  RangeE.MergeCells:= true;
  ...
  RangeE:= IExcelApp.Range['A1', semCols[Length(arColumns) - (NoAudRowCount - 1)] + IntToStr(CycleRow + 6)];
...
//  RangeE.HorizontalAlignment:= xlCenter;
  ...
  tempCSL.Clear;
  if (KafIK = 1) then
    CycleDataSet.CommandText:= 'SELECT * FROM ckl_disc WHERE ik_ckl_disc in (SELECT ik_ckl_disc FROM sv_disc WHERE (ik_uch_plan = ' + IntToStr(UchPlanIK) + ') and (ik_disc in (SELECT ik_disc FROM discpln WHERE ik_type_disc = 1)))'
  else CycleDataSet.CommandText:= 'SELECT * FROM ckl_disc WHERE ik_ckl_disc in (SELECT ik_ckl_disc FROM sv_disc WHERE (ik_uch_plan = ' + IntToStr(UchPlanIK) + ') and (ik_disc in (SELECT ik_disc FROM discpln WHERE ik_type_disc = 1)) and (ik_disc_uch_plan in (Select ik_disc_uch_plan From Content_UchPl Where ik_kaf = ' + IntToStr(KafIK) + ')))';
  CycleDataSet.Open;
  CycleDataSet.First;
  ...
  begin
    cklZE := 0;
    if (KafIK = 1) then
      GroupDataSet.CommandText:= 'SELECT * FROM grp_disc WHERE IK_grp_disc in (SELECT IK_grp_disc FROM sv_disc WHERE (ik_uch_plan = ' + IntToStr(UchPlanIK) + ') and (ik_disc in (SELECT ik_disc FROM discpln WHERE ik_type_disc = 1)) and (ik_ckl_disc = ' + CycleDataSet.FieldByName('ik_ckl_disc').AsString + '))'
    else GroupDataSet.CommandText:= 'SELECT * FROM grp_disc WHERE IK_grp_disc in (SELECT IK_grp_disc FROM sv_disc WHERE (ik_uch_plan = ' + IntToStr(UchPlanIK) + ') and (ik_disc in (SELECT ik_disc FROM discpln WHERE ik_type_disc = 1)) and (ik_ckl_disc = ' + CycleDataSet.FieldByName('ik_ckl_disc').AsString + ') and (ik_disc_uch_plan in (Select ik_disc_uch_plan From Content_UchPl Where ik_kaf = ' + IntToStr(KafIK) + ')))';
    GroupDataSet.Open;
    GroupDataSet.First;
    if CycleDataSet.FieldByName('ik_ckl_disc').AsInteger <> 15 then
    begin
      RangeE:= IExcelApp.Range['B' + IntToStr(CycleRow), 'B' + IntToStr(CycleRow)];
      if (CycleDataSet.FieldByName('Ccode_ckl_disc').Value <> NULL) then
        RangeE.Value2:= Trim(CycleDataSet.FieldByName('Ccode_ckl_disc').AsString) + '.00';

      RangeE:= IExcelApp.Range['C' + IntToStr(CycleRow), 'C' + IntToStr(CycleRow)];
      RangeE.MergeCells:= true;
      RangeE.Value2:= Trim(CycleDataSet.FieldByName('Cname_ckl_disc').AsString);
      RangeE.Font.Bold:= true;
      RangeE.Font.Size:= 13;
      GroupRow:= CycleRow + 1;

    end
    else GroupRow:= CycleRow;
    RowInCycle:= 0;
    tempSL.Clear;
    ...
    begin
      ...
      begin
        RangeE:= IExcelApp.Range['B' + IntToStr(GroupRow), 'B' + IntToStr(GroupRow)];

        if (CycleDataSet.FieldByName('Ccode_ckl_disc').Value <> NULL) and (GroupDataSet.FieldByName('Ccode_grp_disc').Value <> NULL) then
          RangeE.Value2:= Trim(CycleDataSet.FieldByName('Ccode_ckl_disc').AsString) + '.' + Trim(GroupDataSet.FieldByName('Ccode_grp_disc').AsString) + '.00';
        RangeE:= IExcelApp.Range['C' + IntToStr(GroupRow), 'C' + IntToStr(GroupRow)];
        RangeE.MergeCells:= true;

        RangeE.Value2:= Trim(GroupDataSet.FieldByName('Cname_grp_disc').AsString);
        RangeE.Font.Bold:= true;
        RangeE.Font.Italic:= true;
        RangeE.Font.Size:= 12;

      end
      else GroupRow:= GroupRow-1;

      RowInGroup:= 0;                                                                                                                                                                                                                                                                                                                       // исправил - было Cname_disc
      DiscDataSet.CommandText:= 'Select * From sv_disc INNER JOIN discpln ON sv_disc.ik_disc = discpln.ik_disc Where (sv_disc.ik_grp_disc = ' + GroupDataSet.FieldByName('ik_grp_disc').AsString + ') and (sv_disc.ik_ckl_disc = ' + CycleDataSet.FieldByName('ik_ckl_disc').AsString + ') and (ik_uch_plan = ' + IntToStr(UchPlanIK) + ') Order By cname_ckl_disc1';
      DiscDataSet.Open;
      while (not DiscDataSet.Eof) do
      begin
        RangeE:= IExcelApp.Range['B' + IntToStr(GroupRow + RowInGroup + 1), 'B' + IntToStr(GroupRow + RowInGroup + 1)];
        RangeE.Value2:= Trim(DiscDataSet.FieldByName('cname_ckl_disc1').AsString);
        RangeE:= IExcelApp.Range['C' + IntToStr(GroupRow + RowInGroup + 1), 'C' + IntToStr(GroupRow + RowInGroup + 1)];
        RangeE.Value2:= Trim(DiscDataSet.FieldByName('cname_disc').AsString);

        dm.qContentUchPlan.SQL.Clear;
        if (KafIK = 1) then
          dm.qContentUchPlan.SQL.Add('Select * From Content_UchPl Where ik_disc_uch_plan = ' + DiscDataSet.FieldByName('ik_disc_uch_plan').AsString)
        else
          dm.qContentUchPlan.SQL.Add('Select * From Content_UchPl Where (ik_disc_uch_plan = ' + DiscDataSet.FieldByName('ik_disc_uch_plan').AsString + ') and (ik_kaf = ' + IntToStr(KafIK) + ')');
        dm.qContentUchPlan.Open;

        slColumnValues.Clear;
        getColumnsValues(DiscDataSet.FieldByName('ik_disc_uch_plan').AsInteger, lect, lab, pract, slColumnValues, nil, true);

       

        RangeE:= IExcelApp.Range[semCols3[1] + IntToStr(GroupRow + RowInGroup + 1), semCols3[1] + IntToStr(GroupRow + RowInGroup + 1)];
        RangeE.Value2:= DiscDataSet.FieldByName('iHour_gos').AsInteger;
        RangeE:= IExcelApp.Range['L' + IntToStr(GroupRow + RowInGroup + 1), 'L' + IntToStr(GroupRow + RowInGroup + 1)];

        if ZE>0 then
        
        if DiscDataSet.FieldByName('ik_disc').AsInteger<>FISCULTURA then
        begin
           RangeE.Value2:= DiscDataSet.FieldByName('iHour_gos').AsInteger/ZE;
            grpZE := grpZE + Round(DiscDataSet.FieldByName('iHour_gos').AsInteger/ZE);
        end
          else
          begin
            RangeE.Value2:= 2;
        ...
        RangeE:= IExcelApp.Range[semCols3[2] + IntToStr(GroupRow + RowInGroup + 1), semCols3[2] + IntToStr(GroupRow + RowInGroup + 1)];
        RangeE.Value2:= IntToStr(lect);
        RangeE:= IExcelApp.Range[semCols3[3] + IntToStr(GroupRow + RowInGroup + 1), semCols3[3] + IntToStr(GroupRow + RowInGroup + 1)];
        RangeE.Value2:= IntToStr(lab);
        RangeE:= IExcelApp.Range[semCols3[4] + IntToStr(GroupRow + RowInGroup + 1), semCols3[4] + IntToStr(GroupRow + RowInGroup + 1)];
        RangeE.Value2:= IntToStr(pract);
        RangeE:= IExcelApp.Range[semCols3[5] + IntToStr(GroupRow + RowInGroup + 1), semCols3[5] + IntToStr(GroupRow + RowInGroup + 1)];
        RangeE.Value2:= IntToStr(pract + lab + lect);
        RangeE:= IExcelApp.Range[semCols3[6] + IntToStr(GroupRow + RowInGroup + 1), semCols3[6] + IntToStr(GroupRow + RowInGroup + 1)];
        RangeE.Value2:= DiscDataSet.FieldByName('iIndivid').AsInteger;
        RangeE:= IExcelApp.Range[semCols3[7] + IntToStr(GroupRow + RowInGroup + 1), semCols3[7] + IntToStr(GroupRow + RowInGroup + 1)];
        i:= DiscDataSet.FieldByName('iHour_gos').AsInteger - (pract + lab + lect) - DiscDataSet.FieldByName('iIndivid').AsInteger;
        RangeE.Value2:= IntToStr(i);

        //вывод компетенций
        tempStr := '';
        CompetDataSet.Filter :='ik_disc_uch_plan='+DiscDataSet.FieldByName('ik_disc_uch_plan').AsString;
        CompetDataSet.Filtered := true;
        CompetDataSet.First;
        while not CompetDataSet.Eof do
        begin
          if tempStr<>'' then tempStr := tempStr + ', ';
          tempStr := tempStr + CompetDataSet.FieldByName('short_Name').AsString;
          CompetDataSet.Next;
        end;
        RangeE:= IExcelApp.Range[semCols[6*year_count+1] + IntToStr(GroupRow + RowInGroup + 1), semCols[6*year_count+1] + IntToStr(GroupRow + RowInGroup + 1)];
       ...
       //   if i div 3 =0 then sem_ind = i+1 else
          ...
          case (arColumns[i].columnType) of
            3: begin
                  RangeE:= IExcelApp.Range[semCols2[i+1] + IntToStr(GroupRow + RowInGroup + 1), semCols2[i+1] + IntToStr(GroupRow + RowInGroup + 1)];
            ...
            11: begin
                 ...
                 for j := 0 to 2 do
                 begin
                   RangeE:= IExcelApp.Range[semCols[sem_ind*3+1+j] + IntToStr(GroupRow + RowInGroup + 1), semCols[sem_ind*3+1+j] + IntToStr(GroupRow + RowInGroup + 1)];
                   if j=2 then
                            RangeE.Value2:= copy(slColumnValues[i],1,Length(slColumnValues[i]))
                     else
                     begin
                       RangeE.Value2:= copy(slColumnValues[i],1,Pos('\',slColumnValues[i])-1);
                       slColumnValues[i] := copy(slColumnValues[i],Pos('\',slColumnValues[i])+1,Length(slColumnValues[i])-Pos('\',slColumnValues[i]));
                     end;
                 end;
                 inc(sem_ind);
                end;
        ...
        begin
          ...
          begin
            if (dm.qContentUchPlan.FieldByName('ik_kaf').Value <> NULL) then
              if (slKafedraList.IndexOf(dm.qContentUchPlan.FieldByName('ik_kaf').AsString) = -1) then
                slKafedraList.Add(dm.qContentUchPlan.FieldByName('ik_kaf').AsString);
            dm.qContentUchPlan.Next;
          end;
        end
        else slKafedraList.Add(IntToStr(KafIK));
        RangeE:= IExcelApp.Range['A' + IntToStr(GroupRow + RowInGroup + 1), 'A' + IntToStr(GroupRow + RowInGroup + 1)];
        RangeE.Value2:= GetValuesFromStringList(@slKafedraList, 'kafedra','ik_kaf','cshort_name_kaf');
        if GroupDataSet.FieldByName('ik_grp_disc').AsInteger = 14 then
      ...
      if GroupDataSet.FieldByName('ik_grp_disc').AsInteger <> 14 then
      begin
        RangeE:= IExcelApp.Range[semCols3[1] + IntToStr(GroupRow), semCols3[1] + IntToStr(GroupRow)];
        RangeE.Value2:= '=SUM(' + semCols3[1] + IntToStr(GroupRow + 1) + ':' + semCols3[1] + IntToStr(GroupRow + RowInGroup) + ')';

        RangeE.Font.Bold:= true;
        RangeE.Font.Size:= 12;

        RangeE:= IExcelApp.Range['L' + IntToStr(GroupRow), 'L' + IntToStr(GroupRow)];
        if ZE>0 then
          RangeE.Value2:= IntToStr(grpZE);//'='+semCols3[1] + IntToStr(GroupRow)+'/'+IntToStr(ZE);
        ...
        for i:= 1 to 7 do
        begin
          RangeE:= IExcelApp.Range[semCols3[i] + IntToStr(GroupRow), semCols3[i] + IntToStr(GroupRow)];
          RangeE.Value2:= '=SUM(' + semCols3[i] + IntToStr(GroupRow - RowInGroup) + ':' + semCols3[i] + IntToStr(GroupRow - 1) + ')';
      ...
      else RowInCycle:= RowInCycle + RowInGroup;
      GroupDataSet.Next;
    end;
    GroupDataSet.Close;
    ...
    if CycleDataSet.FieldByName('ik_ckl_disc').AsInteger <> 15 then
    begin
      RangeE:= IExcelApp.Range[semCols3[1] + IntToStr(CycleRow), semCols3[1] + IntToStr(CycleRow)];
      tempStr:= '';
      ...
      RangeE:= IExcelApp.Range['L' + IntToStr(CycleRow), 'L' + IntToStr(CycleRow)];
      ...
      if ZE>0 then RangeE.Value2:=IntToStr(cklZE);
      ...
      for i:= 1 to 7 do
      begin
        RangeE:= IExcelApp.Range[semCols3[i] + IntToStr(CycleRow), semCols3[i] + IntToStr(CycleRow)];
        tempStr:= '';
        for j:= 0 to tempSL.Count-1 do
    ...
    else CycleRow:= CycleRow + RowInCycle;
    CycleDataSet.Next;
  end;
  CycleDataSet.Close;
  ...
  for i:= 1 to 7 do
   ...
   RangeE:= IExcelApp.Range['L' + IntToStr(CycleRow), 'L' + IntToStr(CycleRow)];
   if ZE>0 then RangeE.Value2 := IntToStr(allZE);
  ...
  begin
    RangeE:= IExcelApp.Range[semCols[1] + '11', semCols[Length(arColumns) - NoAudRowCount] + IntToStr(CycleRow)];
    RangeE.HorizontalAlignment:= xlCenter;
    RangeE.VerticalAlignment:= xlCenter;
  end;
  RangeE:= IExcelApp.Range['A4', semCols[6*year_count+1] + IntToStr(CycleRow)];
  RangeE.Columns.AutoFit;
  RangeE.Borders.LineStyle:= 1;

  DiscDataSet.CommandText:= 'SELECT cName_disc, iHour_gos, ik_disc_uch_plan FROM sv_disc INNER JOIN discpln ON sv_disc.ik_disc = discpln.ik_disc WHERE (ik_type_disc <> 1) and (ik_uch_plan = ' + IntToStr(UchPlanIK) + ')';
  DiscDataSet.Open;
  if DiscDataSet.RecordCount > 0 then
  begin
    RangeE:= IExcelApp.Range['A' + IntToStr(CycleRow + 2), 'A' + IntToStr(CycleRow + 2)];
    RangeE.Value2:= 'Название практики/работы';
    RangeE:= IExcelApp.Range['B' + IntToStr(CycleRow + 2), 'B' + IntToStr(CycleRow + 2)];
    RangeE.Value2:= 'Семестр';
    RangeE:= IExcelApp.Range['C' + IntToStr(CycleRow + 2), 'C' + IntToStr(CycleRow + 2)];
    RangeE.Value2:= 'Количество недель';
    RangeE:= IExcelApp.Range['A' + IntToStr(CycleRow + 2), 'C' + IntToStr(CycleRow + 2)];
    RangeE.Font.Bold:= 1;
    RangeE.Font.Size:= 12;
    RangeE.HorizontalAlignment:= xlCenter;
    DiscDataSet.First;
    while not DiscDataSet.Eof do
    begin
      RangeE:= IExcelApp.Range['A' + IntToStr(CycleRow + 2 + DiscDataSet.RecNo), 'A' + IntToStr(CycleRow + 2 + DiscDataSet.RecNo)];
      RangeE.Value2:= DiscDataSet.FieldByName('cName_disc').AsString;
      CycleDataSet.CommandText:= 'SELECT n_sem FROM Content_UchPl WHERE ik_disc_uch_plan = ' + DiscDataSet.FieldByName('ik_disc_uch_plan').AsString;
      CycleDataSet.Open;
      tempStr:= '';
      while not CycleDataSet.Eof do
      begin
        tempStr:= tempStr + CycleDataSet.FieldByName('n_sem').AsString + ', ';
        CycleDataSet.Next;
      end;
      CycleDataSet.Close;
      RangeE:= IExcelApp.Range['B' + IntToStr(CycleRow + 2 + DiscDataSet.RecNo), 'B' + IntToStr(CycleRow + 2 + DiscDataSet.RecNo)];
      Delete(tempStr, Length(tempStr)-1, 2);
      RangeE.Value2:= tempStr;
      RangeE:= IExcelApp.Range['C' + IntToStr(CycleRow + 2 + DiscDataSet.RecNo), 'C' + IntToStr(CycleRow + 2 + DiscDataSet.RecNo)];
      RangeE.Value2:= DiscDataSet.FieldByName('iHour_gos').AsString;
      DiscDataSet.Next;
    end;
    RangeE:= IExcelApp.Range['A' + IntToStr(CycleRow + 2), 'C' + IntToStr(CycleRow + 2 + DiscDataSet.RecordCount)];
    RangeE.Columns.AutoFit;
    RangeE.Borders.LineStyle:= 1;
  end;                   
  DiscDataSet.Close;
  ...
  begin
    ...
    begin
      if DiscDataSet.Active then DiscDataSet.Close;
      DiscDataSet.Free;
    end;
    if Assigned(GroupDataSet) then
    begin
      if GroupDataSet.Active then GroupDataSet.Close;
      GroupDataSet.Free;
    end;
    if Assigned(CycleDataSet) then
    begin
      if CycleDataSet.Active then CycleDataSet.Close;
      CycleDataSet.Free;
    end;
    if Assigned(SemDataSet) then
    begin
      if SemDataSet.Active then SemDataSet.Close;
      SemDataSet.Free;
    end;
    if Assigned(CompetDataSet) then
    begin
      if CompetDataSet.Active then CompetDataSet.Close;
      CompetDataSet.Free;
    end;
    if Assigned(FgosDataSet) then
    begin
      if FgosDataSet.Active then FgosDataSet.Close;
      FgosDataSet.Free;
    end;
...
begin
  ...
  tempStoredProc.Parameters.CreateParameter('@ik_spclz', ftInteger, pdInput, 0, SpclzIK);
  tempStoredProc.Parameters.CreateParameter('@ik_spec', ftInteger, pdInput, 0, SpecIK);
  tempStoredProc.Parameters.CreateParameter('@Sh_spclz', ftString, pdInput, 10, CodeSpclz);
  tempStoredProc.Parameters.CreateParameter('@Cname_spclz', ftString, pdInput, 150, NameSpclz);
  tempStoredProc.Parameters.CreateParameter('@Cshort_name_spclz', ftString, pdInput, 20, NameShortSpclz);
  try
...
function TUchPlanController.UpdateGrup(UchPlanOld,UchPlanNew,SpecFac : integer): boolean;
var
  DataSet: TADOStoredProc;
begin
  DataSet:= TADOStoredProc.Create(nil);
  ...
  (DataSet as TADOStoredProc).Parameters.CreateParameter('@ik_uch_plan_old', ftInteger, pdInput,  0, UchPlanOld);
  (DataSet as TADOStoredProc).Parameters.CreateParameter('@ik_uch_plan_new', ftInteger, pdInput,  0, UchPlanNew);
  (DataSet as TADOStoredProc).Parameters.CreateParameter('@ik_spec_fac', ftInteger, pdInput, 0, SpecFac);
  try
    DataSet.ExecProc;
  except
    DataSet.Free;
...
begin
  ...
  (DataSet as TADOStoredProc).Parameters.CreateParameter('@ik_grup', ftInteger, pdInput,  0, ik_grup);
  (DataSet as TADOStoredProc).Parameters.CreateParameter('@ik_uch_plan_new', ftInteger, pdInput,  0, UchPlanNew);
  try
    DataSet.ExecProc;
  except
...
begin
  ...
  DataSet.Connection:= dm.DBConnect;
  DataSet.CommandType:= cmdText;
  DataSet.CommandText:= 'SELECT ik_spclz from Uch_pl where ik_uch_plan = (select ik_uch_plan from grup where Ik_grup=' + IntToStr(IK_grup) + ')';
  DataSet.Open;
...
begin
  ...
  DataSet.Connection:= dm.DBConnect;
  DataSet.CommandType:= cmdText;
  DataSet.CommandText:= 'SELECT ik_form_ed from Uch_pl where ik_uch_plan = (select ik_uch_plan from grup where Ik_grup=' + IntToStr(IK_grup) + ')';
  DataSet.Open;
...
begin
  ...
  DataSet.Connection:= dm.DBConnect;
  DataSet.CommandType:= cmdText;
  DataSet.CommandText:= 'SELECT ik_year_uch_pl from Uch_pl where ik_uch_plan = (select ik_uch_plan from grup where Ik_grup=' + IntToStr(IK_grup) + ')';
  DataSet.Open;
...
function TUchPlanController.GetSemesterHourityString(semesterNumber: integer): string;
var
  i: integer;
begin
  Result:= '00\00\00';
  for i := 0 to Length(semesters) - 1 do
    ...
    begin
      ...
      begin
        ...
        else Result:= '0' + IntToStr(semesters[i].auditorHour[0]) + '\';
      end
      else Result:= '00\';
      ...
      begin
        ...
        else Result:= Result + '0' + IntToStr(semesters[i].auditorHour[1]) + '\';
      end
      ...
      begin
        ...
        else Result:= Result + '0' + IntToStr(semesters[i].auditorHour[2]);
      end
      else Result:= Result + '00';
      exit;
    end;
...
{  n:= 1;
  ...
  Add(TVidZanyatException.Create(AVidZanyatIK, ASemNumber, AKafIK, ANumber, ACount));
end;
...
begin
  ...
  while (i < Count) do
  begin
    if (TVidZanyatException(Items[i]).SemNumber = ASemNumber) then Delete(i)
    else inc(i);
  end;
end;
...
begin
  ...
  while (i < Count) do
  begin
    if (TVidZanyatException(Items[i]).VidZanyatIK = AVidZanyat) then Delete(i)
    else inc(i);
  end;
end;
...
function TVidZanyatExceptionList.GetException(
  Index: Integer): TVidZanyatException;
begin
...
begin
  ...
  for I:= 0 to Count - 1 do
    if (Items[i].VidZanyatIK = AVidZanyatIK) and (Items[i].SemNumber = ASemNumber) and
      (Items[i].KafIK = AKafIK) and (Items[i].Number = ANumber) then
    begin
...
begin
  ...
  for I:= 0 to Count - 1 do
    if (Items[i].VidZanyatIK = AVidZanyatIK) and (Items[i].SemNumber = ASemNumber) and (Items[i].KafIK = AKafIK) then
    begin
      Result:= Items[i].Count;
      exit;
    end;
...
constructor TVidZanyatException.Create(AVidZanyatIK, ASemNumber, AKafIK, ANumber,
  ACount: integer);
begin
...
function TSemKafList.GetNextSemKafNumber(AVidZanyatIK, ASemNumber, AKafIK: integer): integer;
var
  i: integer;
begin
  Result:= 0;
    ...
    try
      ...
      begin
        ...
        if dataset.FieldByName('ik_disc_uch_plan').AsInteger<>0 then
          ik_disc_uch_plan_temp:=dataset.FieldByName('ik_disc_uch_plan').AsInteger
        else
...
begin
 ...
 { try
  ...
  dm.qContentUchPlan_temp.SQL.Add('Select * From Content_UchPl Where ik_disc_uch_plan = ' + IntToStr(new_ik_disc_uch_plan));
  ...
  begin
    ...
    dm.qContentUchPlan_temp.FieldByName('iK_disc_uch_plan').Value:= new_ik_disc_uch_plan;
    dm.qContentUchPlan_temp.FieldByName('iK_vid_zanyat').Value:= dm.qContentUchPlan.FieldByName('iK_vid_zanyat').Value;
    dm.qContentUchPlan_temp.FieldByName('i_hour_per_week').Value:= dm.qContentUchPlan.FieldByName('i_hour_per_week').Value;
    dm.qContentUchPlan_temp.FieldByName('n_sem').Value:= dm.qContentUchPlan.FieldByName('n_sem').Value;
    dm.qContentUchPlan_temp.FieldByName('ik_kaf').Value:= dm.qContentUchPlan.FieldByName('ik_kaf').Value;
  ...
  tempDS_new.CommandText:= 'SELECT * FROM UchPlan_WeekCount_Exception WHERE ik_upContent in (SELECT ik_upContent FROM Content_UchPl WHERE ik_disc_uch_plan = ' + IntToStr(new_ik_disc_uch_plan) + ')';
  tempDS_new.open;
  ...
  begin
    ...
    while not tempDS.Eof do
    begin
      if dm.qContentUchPlan.Locate('ik_upcontent', tempDS.FieldByName('ik_upcontent').value, [loPartialKey]) then
        if dm.qContentUchPlan_temp.Locate('ik_vid_zanyat; n_sem; ik_kaf; i_hour_per_week',
                  VarArrayOf([dm.qContentUchPlan.FieldByName('iK_vid_zanyat').Value,
                     ...
                     dm.qContentUchPlan.FieldByName('ik_kaf').Value]), [loPartialKey]) then

        begin
          tempDS_new.Insert;
          tempDS_new.FieldByName('ik_upContent').AsInteger:= dm.qContentUchPlan_temp.FieldByName('ik_upContent').AsInteger;
          tempDS_new.FieldByName('week_count').AsInteger:= tempDS.FieldByName('week_count').AsInteger;
          tempDS_new.Post;
        end;
  ...
  if tempDS.Active then tempDS.Close;
  if tempDS_new.Active then tempDS_new.Close; 
  FreeAndNil(tempDS);
  ...
  if dm.qContentUchPlan.Active then dm.qContentUchPlan.Close;
  if dm.qContentUchPlan_temp.Active then dm.qContentUchPlan_temp.Close;
...
var
  ...
  IK_vz_List, IK_vz_List_old, IK_nsem_List, ik_kaf_list, IK_nsem_List_old, ik_kaf_list_old: TStringList;
  i, j: integer;
  vz_chek: boolean;
begin
...
{  DataSet:= TGeneralController.Instance.GetNewADOStoredProc('UpdateDiscInUchPlan318', false);
  ...
  dm.qContentUchPlan_temp.Close;
  dm.qContentUchPlan_temp.SQL.Clear;
  dm.qContentUchPlan_temp.SQL.Add('Select * From Content_UchPl Where ik_disc_uch_plan = ' + IntToStr(ik_disc_uch_plan_temp));
  dm.qContentUchPlan_temp.ExecSQL;
  dm.qContentUchPlan_temp.Open;
  dm.qContentUchPlan_temp.First;
  ...
  begin
    ...
    if ik_vz_List.Count>0 then
      for i:=0 to ik_vz_List.Count-1 do
        if dm.qContentUchPlan.FieldByName('ik_vid_zanyat').AsString=ik_vz_list[i] then
        begin
          vz_chek:=true;
          break;
        end;
    if not vz_chek then
      IK_vz_List.Add(dm.qContentUchPlan.FieldByName('ik_vid_zanyat').AsString);
    dm.qContentUchPlan.Next;
  end;

  IK_vz_List_old:= TStringList.Create;
  dm.qContentUchPlan_temp.First;      //прочитаем виды занятий которые есть в таблице
  while (not dm.qContentUchPlan_temp.Eof) do
  begin
    vz_chek:=false;
    if ik_vz_List_old.Count>0 then
      for i:=0 to ik_vz_List_old.Count-1 do
        if dm.qContentUchPlan_temp.FieldByName('ik_vid_zanyat').AsString=ik_vz_list_old[i] then
        begin
          vz_chek:=true;
          break;
        end;
    if not vz_chek then
      IK_vz_List_old.Add(dm.qContentUchPlan_temp.FieldByName('ik_vid_zanyat').AsString);
  ...
  begin
    ...
    begin
      IK_nsem_List.Clear;
      ik_kaf_list.Clear;
      dm.qContentUchPlan.First;      //прочитаем номера семестров которые есть в таблице
      while (not dm.qContentUchPlan.Eof) do
      begin
        if dm.qContentUchPlan.FieldByName('ik_vid_zanyat').AsString=ik_vz_list[i] then
        begin
      ...
      dm.qContentUchPlan_temp.First;      //прочитаем номера семестров которые есть в таблице
      while (not dm.qContentUchPlan_temp.Eof) do
      begin
        if dm.qContentUchPlan_temp.FieldByName('ik_vid_zanyat').AsString=ik_vz_list[i] then
        begin
          ik_nsem_list_old.add(dm.qContentUchPlan_temp.FieldByName('n_sem').AsString);
          ik_kaf_list_old.add(dm.qContentUchPlan_temp.FieldByName('ik_kaf').AsString);
        end;
        dm.qContentUchPlan_temp.Next;
      end;
      ...
      begin
        ...
        //for j:=0 to IK_nsem_List.Count-1 do
          ...
          begin
            if (IK_nsem_List_old.IndexOf(IK_nsem_List[j])<>-1)    //удаление повторений
              and (IK_kaf_List_old[IK_nsem_List_old.IndexOf(IK_nsem_List[j])]=IK_kaf_List[j]) then
            begin
              IK_kaf_List_old.Delete(IK_nsem_List_old.IndexOf(IK_nsem_List[j]));
              IK_kaf_List.Delete(j);
              IK_nsem_List_old.Delete(IK_nsem_List_old.IndexOf(IK_nsem_List[j]));
          ...
          begin
            ...
            begin
              if not dm.qContentUchPlan_temp.Locate('ik_vid_zanyat; n_sem; ik_kaf', VarArrayOf([ik_vz_list[i], ik_nsem_list[0], ik_kaf_list[0]]), [loPartialKey]) then
                if dm.qContentUchPlan_temp.Locate('ik_vid_zanyat; n_sem; ik_kaf', VarArrayOf([ik_vz_list[i], ik_nsem_list_old[0], ik_kaf_list_old[0]]), [loPartialKey]) then
                begin                          //редактирование прежних записей
                  dm.qContentUchPlan_temp.Edit;
                  dm.qContentUchPlan_temp.FieldByName('n_sem').Value:= ik_nsem_list[0];
                  dm.qContentUchPlan_temp.FieldByName('ik_kaf').Value:= ik_kaf_list[0];
                  dm.qContentUchPlan_temp.post;
            ...
            begin               //вставка новой, если такой еще нет
              ...
              begin
                dm.qContentUchPlan_temp.insert;
                dm.qContentUchPlan_temp.FieldByName('ik_vid_zanyat').Value:= ik_vz_list[i];
                dm.qContentUchPlan_temp.FieldByName('ik_disc_uch_plan').Value:= ik_disc_uch_plan_temp;
                dm.qContentUchPlan_temp.FieldByName('n_sem').Value:= ik_nsem_list[0];
                dm.qContentUchPlan_temp.FieldByName('ik_kaf').Value:= ik_kaf_list[0];
                dm.qContentUchPlan_temp.post;
              end;
        ...
        begin
          ...
          while j<>IK_nsem_List.Count do
          begin
            dm.qContentUchPlan.Locate('ik_vid_zanyat; n_sem; ik_kaf', VarArrayOf([ik_vz_list[i], ik_nsem_list[j], ik_kaf_list[j]]), [loPartialKey]);
            dm.qContentUchPlan_temp.Locate('ik_vid_zanyat; n_sem; ik_kaf', VarArrayOf([ik_vz_list[i], ik_nsem_list_old[IK_nsem_List_old.IndexOf(IK_nsem_List[j])], ik_kaf_list_old[IK_nsem_List_old.IndexOf(IK_nsem_List[j])]]), [loPartialKey]);
            if (IK_nsem_List_old.IndexOf(IK_nsem_List[j])<>-1)    //удаление повторений
              and (IK_kaf_List_old[IK_nsem_List_old.IndexOf(IK_nsem_List[j])]=IK_kaf_List[j])
              and (dm.qContentUchPlan.FieldByName('i_hour_per_week').AsInteger=dm.qContentUchPlan_temp.FieldByName('i_hour_per_week').AsInteger)
               then
            begin
              IK_kaf_List_old.Delete(IK_nsem_List_old.IndexOf(IK_nsem_List[j]));
              IK_kaf_List.Delete(j);
          ...
          while IK_nsem_List.Count>0 do         //оставшиеся в новом списке
          begin
            if dm.qContentUchPlan.Locate('ik_vid_zanyat; n_sem; ik_kaf', VarArrayOf([ik_vz_list[i], ik_nsem_list[0], ik_kaf_list[0]]), [loPartialKey]) then
              i_hour_per_week:=dm.qContentUchPlan.FieldByName('i_hour_per_week').AsInteger
            else
              i_hour_per_week:=0;
            ...
            begin
                ...
                begin                          //редактирование прежних записей
                  ...
                  if i_hour_per_week<>0 then
                    dm.qContentUchPlan_temp.FieldByName('i_hour_per_week').Value:= i_hour_per_week;
                  dm.qContentUchPlan_temp.post;
    ...
    begin
      while dm.qContentUchPlan_temp.Locate('ik_vid_zanyat', ik_vz_list_old[0], [loPartialKey]) do
        dm.qContentUchPlan_temp.Delete;
  ...
  tempDS_new:= TGeneralController.Instance.GetNewADODataSet(true);
  tempDS_new.CommandText:= 'SELECT * FROM UchPlan_WeekCount_Exception WHERE ik_upContent in (SELECT ik_upContent FROM Content_UchPl WHERE ik_disc_uch_plan = ' + IntToStr(ik_disc_uch_plan_temp) + ')';
  tempDS_new.open;
