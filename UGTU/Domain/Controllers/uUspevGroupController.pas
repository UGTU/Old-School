...
type
  ...
  public
    ...
    function GetListOfVeds(nsem, ik_group, fac_ik: Integer;
      var havePredms: boolean): boolean;
    ...
    procedure ApplyVed(VedNum: string; flag, ik_group, ikVidZan, nsem, ikPredm,
      ikVidExam: Integer; bitClose, bitNapr, bitVnost: boolean;
      ikPrepod, ikVed, ik_upContent: variant; DateExam: TDateTime);
    ...
    // экспорт справок
    function BuildSpravka2014(_ikStudGrup, _type_spr, num: Integer)
      : TReportBase;
...
begin
  ...
  try
    ...
    else if (dmUspevaemost.adospGetAllVidZanyat.Active) then
    begin
      ...
      begin
        ...
        if (not dmUspevaemost.adospGetAllVidZanyat.FieldByName('IsCreated')
          ...
          .AsBoolean) and
          (dmUspevaemost.adospGetAllVidZanyat.FieldByName('IsDependent')
          .AsBoolean) then
        begin
          CreateDependVeds(nsem, ik_group,
            dmUspevaemost.adospGetAllVidZanyat.FieldByName('ik_vid_zanyat')
            .AsInteger);
        end;
...
begin
  try
    Result := TGeneralController.Instance.getDataSetValues
      (@dmUspevaemost.adospSelVedGroup, 'select * from GetSmallVedForGrup(' +
      IntToStr(ikVed) + ')', 'ik_zach', false, null);
  except
    on E: Exception do
    begin
      raise EApplicationException.Create
...
begin
  TApplicationController.GetInstance.AddLogEntry
    ('Сохранение оценок и тем курсовых.');
  ...
  try
    ...
    begin
      ...
      // если это экзамен, то обязателен допуск
        ...
        (dmUspevaemost.adospSelVedGroup.FieldByName('dopusc').Value = true)))
      then
      begin
        ...
        begin
          ...
          CreateParameter('@Ik_zach', ftInteger, pdInput, 0,
            dbgrdVed.Fields[5].Value);
          if (dbgrdVed.Fields[8].Value <> null) then
            CreateParameter('@Cosenca', ftInteger, pdInput, 0,
              dbgrdVed.Fields[8].Value)
          else
            CreateParameter('@Cosenca', ftInteger, pdInput, 0, -1);
          CreateParameter('@cTema', ftString, pdInput, 2000, '');
          CreateParameter('@i_balls', ftInteger, pdInput, 0,
            dbgrdVed.Fields[7].Value)
        end;
...
procedure TUspevGroupController.ApplyVed(VedNum: string;
  flag, ik_group, ikVidZan, nsem, ikPredm, ikVidExam: Integer;
  bitClose, bitNapr, bitVnost: boolean; ikPrepod, ikVed, ik_upContent: variant;
  DateExam: TDateTime);
begin
