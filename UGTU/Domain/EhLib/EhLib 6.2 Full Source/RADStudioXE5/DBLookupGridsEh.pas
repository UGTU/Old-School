...
{$IFDEF EH_LIB_16}
    if Style.IsCustomStyleActive then
      AFont.Color := StyleServices.GetStyleFontColor(sfListItemTextSelected)
    else
...
//  else
//   AFont.Color := StyleServices.GetSystemColor(clWindowText);
end;
...
//  if UseRightToLeftAlignment then ChangeBiDiModeAlignment(AAlignment);
  ...
  begin
{$IFDEF EH_LIB_16}
    Canvas.Font.Color := StyleServices.GetStyleFontColor(sfListItemTextSelected);
{$ELSE}
...
{$ENDIF}
  ...
  WriteCellText(DrawColumn, Canvas, ARect, True, 2, 1, S, AAlignment, tlTop, False, False, 0, 0, True);
  if SpecRow.Selected then
  begin
