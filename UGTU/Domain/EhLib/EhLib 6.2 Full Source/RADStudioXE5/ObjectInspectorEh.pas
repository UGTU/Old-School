...
uses
  SysUtils, Classes, Controls, Windows, Graphics, StdCtrls, Messages,
  MemTreeEh, Contnrs, EhLibVCL, TypInfo, DBAxisGridsEh, GridsEh, ToolCtrlsEh,
...
{ TObjectInspectorEh }
  ...
  protected
    ...
    procedure CellMouseClick(Cell: TGridCoord; Button: TMouseButton; Shift: TShiftState; X, Y: Integer); override;
    procedure CellMouseDown(Cell: TGridCoord; Button: TMouseButton; Shift: TShiftState; X, Y: Integer); override;
...
begin
  ...
  begin
    ...
    FObjIns.Options := [goFixedVertLine, goVertLine, goEditing, goAlwaysShowEditor];
...
begin
  ...
  if (FixedInfo.Horz.FixedBoundary - 4 - VertLineWidth < X) and (FixedInfo.Horz.FixedBoundary + 4 > X) then
  begin
    State := gsColSizing;
    SizingPos := FixedInfo.Horz.FixedBoundary;
    SizingOfs := FixedInfo.Horz.FixedBoundary - X - 1;
    Index := 0;
  end;
end;
...
procedure TObjectInspectorEh.CellMouseClick(Cell: TGridCoord;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
  TreeNode: TPropTreeNodeEh;
begin
  inherited CellMouseClick(Cell, Button, Shift, X, Y);
...
procedure TObjectInspectorEh.CellMouseDown(Cell: TGridCoord;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
  TreeNode: TPropTreeNodeEh;
  LeftTreeBorder, RightTreeBorder: Integer;
begin
  inherited CellMouseDown(Cell, Button, Shift, X, Y);
...
begin
  ...
  if (Parent.Instance <> nil) and
     (PropInfo <> nil) and
     IsStoredProp(Parent.Instance, PropInfo) and
     not IsDefaultPropertyValue(Parent.Instance, PropInfo, nil)
  then
    Result := [fsBold];
end;
...
function TInspectorInplaceEditEh.GetPickList: TCustomListbox;
var
