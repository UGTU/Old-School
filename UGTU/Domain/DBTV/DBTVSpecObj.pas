...
function TDBNodeSpecObject.SetADODataset(Connection: TAdoConnection)
  : TADODataSet;
var
  year, month, day: word;
  per_year: integer;
begin
  ...
  per_year := 0;
  if month < 9 then
    per_year := 1; // showmessage (inttostr(month));

  if showActual then
    AdoDataset.CommandText := 'select * from TREE_groups where (ik_spec_fac='''
      + inttostr(ik) + ''')and((''' + inttostr(CurrentYear) +
      '''-NYear_post)<''' + inttostr(StudyYears + per_year) +
      ''') and (dateExit >getdate()) order by NYear_post'
  else
    AdoDataset.CommandText := 'select * from TREE_groups where ik_spec_fac=''' +
      inttostr(ik) + ''' order by NYear_post';
...
begin
  ...
  begin
    ...
    if AdoDataset.FieldValues['Ik_uch_plan'] <> NULL then
      AddChild(AdoDataset.FieldValues['CName_grup'],
        AdoDataset.FieldValues['IK_grup'], AdoDataset.FieldValues['nYear_post'],
        AdoDataset.FieldValues['Ik_uch_plan'], date_cr, date_ex)
    else
      AddChild(AdoDataset.FieldValues['CName_grup'],
        AdoDataset.FieldValues['IK_grup'], AdoDataset.FieldValues['nYear_post'],
        -1, date_cr, date_ex);
