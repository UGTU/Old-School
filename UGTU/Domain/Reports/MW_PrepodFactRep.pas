...
type
  TPrepodFactReport = class(TExcelReportBase)
  private
    //FTotalSteps: Integer;
...
constructor TPrepodFactReport.Create(Aowner:TComponent);
begin
  inherited;
  //FTotalSteps := -1;
end;

procedure TPrepodFactReport.Execute;
var str: string;
    Rang: Variant;
    i, N: integer;
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        if not(tempDataSet1.FieldByName('Bit_plan_fact').AsBoolean) then
        begin
          inc(N);
          ...
          Items[i,15]:= tempDataSet1.FieldByName('DateTransitionInState').Value;
          Items[i,10]:= tempDataSet1.FieldByName('CountFactTime').Value;
          Items[i,16]:= tempDataSet1.FieldByName('Mark').Value;
          ...
          tempDataSet2.CommandText:= 'Select DateTransitionInState From MethodEdition as M inner join MW_State ON M.IDMethodEdition = MW_State.IDMethodEdition '+
                   'where (M.IDMethodEditionPlan = '+ IntToStr(tempDataSet1.FieldByName('IDMethodEdition').AsInteger) +')and (IDStatus = (select IDStatus from dbo.MW_State where IDMethodEdition = M.IDMethodEdition '+
          ...
          tempDataSet2.Open;
          Items[i,14]:= tempDataSet2.FieldByName('DateTransitionInState').Value;
          tempDataSet2.Close;
          inc(i);
        end;
        tempDataSet1.Next;
...
function TPrepodFactReport.GetTotalSteps: Integer;
begin
